...
def _resolve_core_auth_credential() -> tuple[str, bool]:
    ...
    raise HTTPException(
        status_code=503,
        detail=(
            "Core service unavailable: AG_CORE_API_KEY "
            "(or AG_CORE_INTERNAL_TOKEN in non-production) not configured"
        ),
    )


def _core_api_key() -> str:
    token, _is_internal = _resolve_core_auth_credential()
    return token


def _core_proxy_headers(request: Request) -> dict[str, str]:
    token, is_internal = _resolve_core_auth_credential()
    headers = {
        "X-API-Key": token,
        "X-Request-Id": getattr(request.state, "request_id", ""),
    }
    if is_internal:
        headers[_CORE_INTERNAL_CALLER_HEADER] = _CORE_INTERNAL_CALLER_VALUE
    return headers
