...
_AI_TOOL_PATTERNS: list[re.Pattern[str]] = [
    ...
    re.compile(
        r"zapier|make\.com|n8n\.io", re.I
    ),  # automation tools often connecting AI
]
...
def run_oauth_risk(
    *,
    ...
    for assignment in app_role_assignments:
        ...
        if not role_value:
            continue
        principal_name = assignment.get(
            "principalDisplayName", assignment.get("principalId", "")
        )
    ...
    if critical_app_role_grants:
        ...
        summary_lines = [
            f"{app}: {', '.join(perms)}" for app, perms in list(by_app.items())[:5]
        ]
        findings.append(
            {
