...
def _graph_get(access_token: str, path: str, params: dict[str, str] | None = None) -> list[dict[str, Any]]:
    import httpx

    headers = {"Authorization": f"Bearer {access_token}"}
    url: str | None = f"{_GRAPH_BASE_V1}{path}"
    if params:
    ...
    while url and pages < 20:
        ...
        if resp.status_code == 403:
            log.info("entra_governance: 403 on %s — endpoint likely requires higher license tier", path)
            return []
...
def _get_role_definitions(access_token: str) -> dict[str, str]:
    try:
        roles = _graph_get(access_token, "/directoryRoles", {"$select": "id,displayName,roleTemplateId"})
        return {r.get("roleTemplateId", r.get("id", "")): r.get("displayName", "") for r in roles}
    except Exception:
        return {}
...
def _analyze_role_assignments(
    ...
    for assignment in role_assignments:
        ...
        role_name = role_def_map.get(role_id, _PRIVILEGED_ROLE_IDS.get(role_id, role_id))

        if role_id == global_admin_role_id:
            permanent_global_admins.append(principal_id)

        if role_id in _PRIVILEGED_ROLE_IDS and (principal_id, role_id) not in pim_protected_principals:
            permanent_privileged.append({"principal_id": principal_id, "role": role_name})
...
def _analyze_access_reviews(review_definitions: list[dict]) -> list[dict]:
    ...
    privileged_scopes = {"roleAssignment", "directoryRole"}
    covers_privileged = any(
        r.get("scope", {}).get("query", "").lower().count("role") > 0
        or r.get("instanceEnumerationScope", {}).get("query", "").lower().count("role") > 0
        for r in review_definitions
    )

    inactive = [
        r for r in review_definitions
...
def _analyze_risky_users(risky_users: list[dict]) -> list[dict]:
    ...
    high_risk = [u for u in risky_users if u.get("riskLevel") == "high" and u.get("riskState") not in ("remediated", "dismissed")]
    medium_risk = [u for u in risky_users if u.get("riskLevel") == "medium" and u.get("riskState") not in ("remediated", "dismissed")]
...
def _analyze_ca_policies(ca_policies: list[dict]) -> list[dict]:
    ...
    def _blocks_legacy_auth(policy: dict) -> bool:
        ...
        block = grant.get("operator") == "OR" and "block" in grant.get("builtInControls", [])
...
def run_entra_governance(
    *,
    ...
    risky_users = _graph_get(
        ...
        {"$select": "id,riskLevel,riskState,riskLastUpdatedDateTime", "$filter": "riskState ne 'remediated'"},
    )
    ...
    return {
        ...
        "summary": {
            **role_summary,
            ...
            "ca_policies_enabled": len([p for p in ca_policies if p.get("state") == "enabled"]),
