...
def _get_all(access_token: str, path: str, params: dict[str, str] | None = None) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    import httpx
    ...
    while url and pages < 20:
        ...
        if resp.status_code in {403, 404}:
            log.info("ai_tool_discovery: optional source unavailable: %s %s", path, resp.status_code)
            return rows, {"status": "skipped", "status_code": resp.status_code, "path": path}
...
def _publisher(sp: dict[str, Any]) -> tuple[str, bool]:
    ...
    publisher = name or sp.get("publisherName") or sp.get("appOwnerOrganizationId") or "unknown"
...
def _graph_role_map(service_principals: list[dict[str, Any]]) -> dict[str, str]:
    for sp in service_principals:
        if sp.get("appId") == _GRAPH_APP_ID:
            return {
                str(role.get("id")): str(role.get("value") or role.get("displayName") or "unknown")
...
def _risk_indicators(tool: dict[str, Any]) -> list[str]:
    ...
    permissions = set(tool["delegated_permissions"]) | set(tool["application_permissions"])
...
def run_ai_tool_discovery(*, access_token: str, tenant_id: str, engagement_id: str) -> dict[str, Any]:
    scan_started = _utc_now()
    apps, apps_status = _get_all(access_token, "/applications", {"$select": "id,appId,displayName,web,identifierUris,createdDateTime"})
    service_principals, sp_status = _get_all(access_token, "/servicePrincipals", {"$select": "id,appId,displayName,verifiedPublisher,publisherName,appOwnerOrganizationId,createdDateTime,appRoles"})
    grants, grants_status = _get_all(access_token, "/oauth2PermissionGrants", {"$select": "clientId,principalId,resourceId,scope,consentType"})
    app_roles, app_roles_status = _get_all(access_token, "/servicePrincipals(appId='00000003-0000-0000-c000-000000000000')/appRoleAssignedTo", {"$select": "principalId,appRoleId,resourceId"})
    signins, signins_status = _get_all(access_token, "/auditLogs/signIns", {"$top": "50", "$select": "appId,appDisplayName,createdDateTime"})
    audit_logs, audit_status = _get_all(access_token, "/auditLogs/directoryAudits", {"$top": "50", "$select": "activityDateTime,activityDisplayName,targetResources"})
    ...
    for grant in grants:
        ...
        if grant.get("principalId"):
            principal_ids_by_sp.setdefault(client_id, set()).add(str(grant["principalId"]))
    ...
    for sp in service_principals:
        ...
        match = match_ai_vendor(display_name=display_name, publisher=publisher, app_id=app_id, domains=domains)
        if not match:
            continue
        ...
        tool = {
            ...
            "app_registration_id": str(app_reg.get("id") or "unknown") if isinstance(app_reg, dict) else "unknown",
        ...
        tool["evidence_refs"] = [f"servicePrincipal:{sp_id}", f"application:{tool['application_id']}"]
        tools.append(tool)

    tools.sort(key=lambda item: (item["vendor"].casefold(), item["tool_name"].casefold(), item["application_id"]))
    findings: list[dict[str, Any]] = []
    overprivileged = [t for t in tools if set(t["risk_indicators"]) & {"files_read_write_all", "mail_read_write", "directory_read_write_all", "broad_scope_access"}]
    if overprivileged:
        findings.append({
    ...
    statuses = (apps_status, sp_status, grants_status, app_roles_status, signins_status, audit_status)
    return {
