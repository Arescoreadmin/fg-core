...
AI_VENDOR_SIGNATURES: tuple[AiVendorSignature, ...] = (
    AiVendorSignature(
        "Microsoft",
        "Microsoft Copilot",
        ("microsoft.com", "copilot.microsoft.com", "graph.microsoft.com"),
        ("microsoft", "microsoft corporation"),
        ("microsoft copilot", "copilot for microsoft 365", "m365 copilot"),
        (),
        ("ai_vendor_detected",),
        ("publisher", "display_name", "domain"),
    ),
    AiVendorSignature(
        "OpenAI",
        "ChatGPT",
    ...
    AiVendorSignature(
        "Google",
        "Gemini",
        ("google.com", "gemini.google.com", "ai.google"),
    ...
    AiVendorSignature(
        "Anysphere",
        "Cursor",
        ("cursor.com", "cursor.sh", "anysphere.co"),
        ("anysphere", "cursor"),
    ...
    AiVendorSignature(
        ...
        ("grammarly", "grammarlygo", "grammarly ai"),
        (),
        ("ai_vendor_detected",),
        ("publisher", "display_name", "domain"),
    ),
    ...
    AiVendorSignature(
        ...
        ("servicenow ai", "now assist", "servicenow"),
        (),
        ("ai_vendor_detected",),
        ("publisher", "display_name", "domain"),
    ),
...
def match_ai_vendor(
    *,
    ...
    for signature in AI_VENDOR_SIGNATURES:
        reasons: list[str] = []
        score = 0
        if norm_app_id and norm_app_id in {
            a.casefold() for a in signature.known_app_ids
        }:
            score += 100
            reasons.append("app_id")
        if norm_publisher and any(
            p and p in norm_publisher for p in map(_norm, signature.known_publishers)
        ):
            score += 35
        ...
        if norm_name:
            ...
            if any(n and n in norm_name for n in names):
                ...
                fuzzy = max(
                    (SequenceMatcher(None, norm_name, n).ratio() for n in names),
                    default=0,
                )
                if fuzzy >= 0.82:
                    score += 25
        ...
        for domain in norm_domains:
            if any(
                domain == _norm_domain(d) or domain.endswith("." + _norm_domain(d))
                for d in signature.known_domains
            ):
                score += 30
    ...
    confidence = (
        "confirmed" if score >= 75 else "probable" if score >= 45 else "suspected"
    )
    return {
