...
def __getattr__(name: str):
    if name in _SCHEMA_EXPORTS:
        from services.timeline_authority import schemas as _m

        return getattr(_m, name)
    if name in _ENGINE_EXPORTS:
        from services.timeline_authority import engine as _m

        return getattr(_m, name)
    if name in _REPOSITORY_EXPORTS:
        from services.timeline_authority import repository as _m
