from __future__ import annotations

from services.plane_registry.models import AuthClass, PlaneDef, RouteException

COMMON_GATES = ("check_plane_registry", "route-inventory-audit")
# /admin is intentionally control-only to avoid multi-plane ambiguity under longest-prefix matching.
ADMIN_PREFIX_POLICY = "control_only"


def ex(
    method: str,
    path: str,
    class_name: str,
    why: str,
    *,
    permanent: bool = True,
    expires_at: str = "",
) -> RouteException:
    return RouteException(
        method=method,
        path=path,
        class_name=class_name,
        justification=why,
        permanent=permanent,
        expires_at=expires_at,
    )


PLANE_REGISTRY: list[PlaneDef] = [
    PlaneDef(
        plane_id="control",
        route_prefixes=(
            "/control-plane",
            "/control/testing",
            "/control-tower",
            "/compliance",
            "/compliance-cp",
            "/enterprise-controls",
            "/frameworks",
            "/control-framework-mappings",
            "/exceptions",
            "/breakglass",
            "/governance",
            "/remediation",
            "/controls",
            "/risk-acceptances",
            "/risk-governance",
            "/field-assessment",
            "/portal",
            "/portal/governance",
            "/portal/engagement",
            "/config",
            "/planes",
            "/health",
            "/status",
            "/internal",
            "/admin",
            "/subscriptions",
            "/_debug",
            "/_legacy",
            "/dev",
            "/rag",
            "/metrics",
            "/signing",
            "/governance-reports",
            "/timeline-authority",
            "/control-effectiveness",
            "/remediation-effectiveness",
            "/governance-chain",
            "/governance-learning",
            "/governance-adaptive-intelligence",
            "/governance-optimization",
            "/cgin/trust",
            "/cgin/transparency",
            "/governance-orchestration",
            "/intelligence",
            "/actor-attribution",
            "/actor-assurance",
            "/api/executive",
        ),
        allowed_dependency_categories=("auth", "tenant", "db", "rate", "breakglass"),
        required_make_targets=("control-plane-check", "plane-registry-spot"),
        required_ci_gates=COMMON_GATES,
        maturity_tag="production-grade",
        required_route_invariants=(
            "auth",
            "scoped",
            "tenant_bound",
            "admin_prefix:control_only",
        ),
        auth_class=AuthClass(
            required_scope_prefixes=(
                "control-plane:",
                "governance:",
                "admin:",
                "compliance:",
                "audit:",
                "keys:",
                "dev:",
                "portal:",
                "actor:",
                "assurance:",
            ),
            require_any_scope=True,
            tenant_binding_required=True,
        ),
        public_routes=(
            ex("GET", "/health", "public", "Unauthenticated health probe."),
            ex(
                "HEAD",
                "/health",
                "public",
                "Unauthenticated health probe for uptime monitoring.",
            ),
            ex("GET", "/health/live", "public", "Kubernetes liveness probe."),
            ex("GET", "/health/ready", "public", "Kubernetes readiness probe."),
            ex("GET", "/health/detailed", "public", "Detailed health probe endpoint."),
            ex("GET", "/status", "public", "Legacy status endpoint."),
            ex("GET", "/v1/status", "public", "Legacy v1 status endpoint."),
            ex(
                "GET",
                "/metrics",
                "allowed_internal",
                "Prometheus scrape endpoint. Internal infrastructure only — "
                "no auth, no tenant data, no customer PII. "
                "Must not be exposed on the public customer-facing ingress.",
            ),
            ex(
                "GET",
                "/signing/public-key",
                "public",
                "Public Ed25519 report signing key endpoint. Required for external "
                "auditor/client verification of signed reports. Returns no tenant data, "
                "no customer data, no secrets, and no private key material.",
            ),
            ex(
                "GET",
                "/portal/engagement/health",
                "public",
                "PR 18.2 Engagement Portal health probe — no auth, no tenant data.",
            ),
            ex(
                "GET",
                "/governance-orchestration/health",
                "public",
                "PR 18.4 Governance Orchestration health probe — no auth, no tenant data.",
            ),
            ex(
                "GET",
                "/intelligence/health",
                "public",
                "PR 18.5 Governance Intelligence health probe — no auth, no tenant data.",
            ),
        ),
        global_routes=(
            ex(
                "GET",
                "/admin/system/health",
                "global_admin",
                "Global operator health view.",
            ),
            ex(
                "GET",
                "/admin/system/internal-authority",
                "global_admin",
                "Internal operator authority status view; protected by admin "
                "scope plus internal gateway and not scoped to a customer tenant.",
            ),
            ex(
                "GET",
                "/admin/system/service-principal",
                "global_admin",
                "Platform service principal status view (PR #586); protected by "
                "admin:read scope plus internal gateway. Returns safe metadata only — "
                "no secrets, no customer data.",
            ),
            ex(
                "POST",
                "/admin/system/service-principal/rotate",
                "global_admin",
                "Rotate platform service principal credential (PR #586); protected by "
                "admin:write scope plus internal gateway.",
            ),
            ex(
                "POST",
                "/admin/system/service-principal/suspend",
                "global_admin",
                "Suspend platform service principal (PR #586); protected by "
                "admin:write scope plus internal gateway.",
            ),
            ex(
                "POST",
                "/admin/system/service-principal/resume",
                "global_admin",
                "Resume suspended platform service principal (PR #586); protected by "
                "admin:write scope plus internal gateway.",
            ),
            ex(
                "POST",
                "/admin/system/service-principal/revoke",
                "global_admin",
                "Permanently revoke platform service principal (PR #586); protected by "
                "admin:write scope plus internal gateway. Irreversible.",
            ),
            ex(
                "GET",
                "/admin/keys/needing-rotation",
                "global_admin",
                "Global operator key rotation report.",
            ),
            ex(
                "GET",
                "/admin/keys/{key_prefix}/rotation-status",
                "global_admin",
                "Global operator key rotation state.",
            ),
            ex(
                "POST",
                "/admin/keys/{key_prefix}/rotate",
                "global_admin",
                "Global operator key rotate action.",
            ),
            ex("GET", "/admin/usage/all", "global_admin", "Global usage summary."),
            ex(
                "POST",
                "/admin/tenants",
                "global_admin",
                "Tenant creation is a system-level operation with no prior tenant context.",
            ),
            ex(
                "GET",
                "/admin/tenants",
                "global_admin",
                "Tenant listing is global platform operator metadata.",
            ),
            ex(
                "GET",
                "/admin/bundles",
                "global_admin",
                "Bundle catalog listing is global platform operator metadata — "
                "lists all policy bundles, not tenant-scoped.",
            ),
            ex(
                "GET",
                "/admin/tenants/{tenant_id}",
                "global_admin",
                "Tenant inspection is global platform operator metadata.",
            ),
            ex(
                "GET",
                "/control-plane/v2/delegation",
                "global_admin",
                "Delegation listing is global governance metadata.",
            ),
            ex(
                "POST",
                "/control-plane/v2/delegation",
                "global_admin",
                "Delegation create is global governance metadata.",
            ),
            ex(
                "DELETE",
                "/control-plane/v2/delegation/{delegation_id}",
                "global_admin",
                "Delegation revoke is global governance metadata.",
            ),
            ex(
                "GET",
                "/control-plane/v2/terminal/allowlist",
                "global_admin",
                "Allowlist is global governance metadata.",
            ),
            ex(
                "GET",
                "/_debug/routes",
                "bootstrap",
                "Debug route catalog; blocked in prod-like mode.",
            ),
            ex(
                "GET",
                "/_legacy/ui_feed/_disabled",
                "bootstrap",
                "Legacy disabled route; blocked in prod-like mode.",
            ),
            ex(
                "POST",
                "/admin/billing/meters",
                "global_admin",
                "P1.5 usage meter catalog creation — global operator action, no tenant scope.",
            ),
            ex(
                "GET",
                "/admin/billing/meters",
                "global_admin",
                "P1.5 usage meter catalog listing — global operator metadata.",
            ),
            ex(
                "PATCH",
                "/admin/billing/meters/{meter_code}",
                "global_admin",
                "P1.5 usage meter catalog update — global operator action, no tenant scope.",
            ),
        ),
    ),
    PlaneDef(
        plane_id="security",
        route_prefixes=("/auth",),
        allowed_dependency_categories=("auth", "tenant", "db", "rate", "breakglass"),
        required_make_targets=("security-regression-gates",),
        required_ci_gates=COMMON_GATES,
        maturity_tag="production-grade",
        required_route_invariants=("auth", "tenant_bound"),
        auth_class=AuthClass(
            required_scope_prefixes=("admin:", "keys:"),
            require_any_scope=True,
            tenant_binding_required=True,
        ),
    ),
    PlaneDef(
        plane_id="rbac",
        route_prefixes=("/rbac",),
        allowed_dependency_categories=("auth", "tenant", "db", "rate"),
        required_make_targets=("security-regression-gates",),
        required_ci_gates=COMMON_GATES,
        maturity_tag="tester-ready",
        required_route_invariants=("auth", "tenant_bound"),
        auth_class=AuthClass(
            required_scope_prefixes=(),
            require_any_scope=False,
            tenant_binding_required=True,
        ),
        global_routes=(
            ex(
                "GET",
                "/rbac/roles",
                "global_admin",
                "RBAC role catalog is tenant-invariant static metadata; role auth enforced by require_role.",
            ),
        ),
    ),
    PlaneDef(
        plane_id="data",
        route_prefixes=(
            "/ingest",
            "/feed",
            "/decisions",
            "/defend",
            "/stats",
            "/billing",
        ),
        allowed_dependency_categories=("auth", "tenant", "db", "rate"),
        required_make_targets=("soc-invariants",),
        required_ci_gates=COMMON_GATES,
        maturity_tag="production-grade",
        required_route_invariants=("auth", "tenant_bound"),
        auth_class=AuthClass(
            required_scope_prefixes=(
                "ingest:",
                "feed:",
                "decisions:",
                "defend:",
                "stats:",
                "admin:",
                "billing:",
            ),
            require_any_scope=True,
            tenant_binding_required=True,
        ),
        global_routes=(
            ex(
                "GET",
                "/stats/debug",
                "bootstrap",
                "Debug stats endpoint is operational bootstrap path.",
            ),
        ),
        bootstrap_routes=(
            ex(
                "POST",
                "/ingest/assessment/orgs",
                "bootstrap",
                "Pre-tenant org enrollment; no tenant context exists before this call completes.",
            ),
            ex(
                "GET",
                "/ingest/assessment/{assessment_id}",
                "bootstrap",
                "UUID-gated assessment read; org not yet enrolled as a tenant.",
            ),
            ex(
                "GET",
                "/ingest/assessment/{assessment_id}/questions",
                "bootstrap",
                "UUID-gated question bank read; org not yet enrolled as a tenant.",
            ),
            ex(
                "PATCH",
                "/ingest/assessment/{assessment_id}/responses",
                "bootstrap",
                "UUID-gated response autosave; org not yet enrolled as a tenant.",
            ),
            ex(
                "POST",
                "/ingest/assessment/{assessment_id}/checkout",
                "bootstrap",
                "UUID-gated Stripe checkout initiation; org not yet enrolled as a tenant.",
            ),
            ex(
                "POST",
                "/ingest/assessment/{assessment_id}/submit",
                "bootstrap",
                "UUID-gated assessment submission; org not yet enrolled as a tenant.",
            ),
            ex(
                "POST",
                "/ingest/assessment/reports/generate",
                "bootstrap",
                "UUID-gated report generation; org not yet enrolled as a tenant.",
            ),
            ex(
                "GET",
                "/ingest/assessment/reports/{report_id}",
                "bootstrap",
                "UUID-gated report status poll; org not yet enrolled as a tenant.",
            ),
            ex(
                "GET",
                "/ingest/assessment/reports/{report_id}/download",
                "bootstrap",
                "UUID-gated report download stub; org not yet enrolled as a tenant.",
            ),
        ),
        auth_exempt_routes=(
            ex(
                "POST",
                "/ingest/assessment/webhooks/stripe",
                "auth_exempt",
                "External Stripe webhook verified by HMAC signature; cannot carry API key credentials.",
            ),
            ex(
                "POST",
                "/billing/webhooks/stripe",
                "auth_exempt",
                "P1.5 billing webhook — verified by Stripe HMAC signature; cannot carry API key credentials.",
            ),
            ex(
                "POST",
                "/billing/usage/events",
                "auth_exempt",
                "P1.5 usage event submission — tenant-bound via require_bound_tenant; "
                "no additional scope needed beyond tenant key.",
            ),
        ),
    ),
    PlaneDef(
        plane_id="agent",
        route_prefixes=("/agent", "/agents"),
        allowed_dependency_categories=("auth", "tenant", "db", "rate"),
        required_make_targets=("agent-unit",),
        required_ci_gates=COMMON_GATES,
        maturity_tag="tester-ready",
        required_route_invariants=("auth", "tenant_bound"),
        auth_class=AuthClass(
            require_any_scope=False,
            tenant_binding_required=True,
            allow_unscoped_keys=False,
        ),
        auth_exempt_routes=(
            ex(
                "POST",
                "/agent/enroll",
                "auth_exempt",
                "Bootstrap enrollment before scoped key issuance.",
            ),
            ex(
                "POST",
                "/agent/cert/enroll",
                "auth_exempt",
                "Bootstrap cert enrollment before scoped key issuance.",
            ),
            ex(
                "POST",
                "/agents/enroll",
                "auth_exempt",
                "R4.10 canonical bootstrap: one-time token IS the credential; consumed atomically.",
            ),
        ),
        global_routes=(
            ex(
                "POST",
                "/agent/cert/renew",
                "bootstrap",
                "Device cert flow binds identity, not tenant.",
            ),
            ex(
                "GET",
                "/agent/cert/status",
                "bootstrap",
                "Cert status is device-level bootstrap metadata.",
            ),
            ex(
                "POST",
                "/agent/commands/ack",
                "bootstrap",
                "Ack uses device identity channel.",
            ),
            ex(
                "POST",
                "/agent/commands/poll",
                "bootstrap",
                "Poll uses device identity channel.",
            ),
            ex(
                "POST",
                "/agent/heartbeat",
                "bootstrap",
                "Heartbeat is device channel metadata.",
            ),
            ex(
                "POST",
                "/agent/key/rotate",
                "bootstrap",
                "Key rotate uses device channel metadata.",
            ),
            ex(
                "POST",
                "/agent/log/anchor",
                "bootstrap",
                "Anchor log uses device channel metadata.",
            ),
            ex(
                "GET",
                "/agent/config",
                "bootstrap",
                "Config fetch uses device channel metadata.",
            ),
            ex(
                "GET",
                "/agent/policy/fetch",
                "bootstrap",
                "Policy fetch uses device channel metadata.",
            ),
            ex(
                "GET",
                "/agent/update/manifest",
                "bootstrap",
                "Update manifest lookup is device channel metadata.",
            ),
            ex(
                "POST",
                "/agent/update/manifest",
                "bootstrap",
                "Update manifest ack is device channel metadata.",
            ),
            ex(
                "POST",
                "/agent/update/report",
                "bootstrap",
                "Update report uses device channel metadata.",
            ),
        ),
    ),
    PlaneDef(
        plane_id="ai",
        route_prefixes=("/ai", "/ai-plane"),
        allowed_dependency_categories=("auth", "tenant", "db", "rate"),
        required_make_targets=("ai-plane-spot",),
        required_ci_gates=COMMON_GATES,
        maturity_tag="tester-ready",
        required_route_invariants=("auth", "tenant_bound", "scoped"),
        auth_class=AuthClass(
            required_scope_prefixes=("compliance:", "ai:", "admin:"),
            require_any_scope=True,
            tenant_binding_required=True,
        ),
    ),
    PlaneDef(
        plane_id="connector",
        route_prefixes=("/modules", "/missions", "/rings", "/roe", "/v1"),
        allowed_dependency_categories=("auth", "tenant", "db", "rate"),
        required_make_targets=("check-connectors-rls",),
        required_ci_gates=COMMON_GATES,
        maturity_tag="experimental",
        required_route_invariants=("auth", "tenant_bound"),
        auth_class=AuthClass(
            required_scope_prefixes=("attestation:",),
            require_any_scope=False,
            tenant_binding_required=True,
            allow_unscoped_keys=True,
        ),
        public_routes=(
            ex("GET", "/v1/status", "public", "Legacy connector status endpoint."),
            ex(
                "GET",
                "/missions",
                "public",
                "Mission catalog is public in current design.",
            ),
            ex(
                "GET",
                "/missions/{mission_id}",
                "public",
                "Mission metadata endpoint is public in current design.",
            ),
            ex(
                "GET",
                "/missions/{mission_id}/status",
                "public",
                "Mission status endpoint is public in current design.",
            ),
            ex(
                "GET",
                "/rings/isolation",
                "public",
                "Ring isolation matrix is public in current design.",
            ),
            ex(
                "GET",
                "/rings/policies",
                "public",
                "Ring policies are public in current design.",
            ),
            ex(
                "POST",
                "/rings/route",
                "public",
                "Ring router is public in current design.",
            ),
            ex(
                "POST",
                "/roe/evaluate",
                "public",
                "ROE evaluate is public in current design.",
            ),
            ex(
                "GET",
                "/roe/policy",
                "public",
                "ROE policy is public in current design.",
            ),
        ),
        global_routes=(
            ex("GET", "/modules", "global_admin", "Global module index."),
            ex(
                "GET", "/modules/{module_id}", "global_admin", "Global module metadata."
            ),
            ex(
                "GET",
                "/modules/enforce/{module_id}",
                "global_admin",
                "Global module enforcement preview.",
            ),
            ex(
                "POST",
                "/modules/register",
                "global_admin",
                "Global module registration.",
            ),
        ),
    ),
    PlaneDef(
        plane_id="evidence",
        route_prefixes=(
            "/audit",
            "/evidence",
            "/approvals",
            "/attestation",
            "/forensics",
            "/verification-requests",
            "/freshness-policies",
            "/freshness",
            "/freshness/snapshots",
            "/freshness/history",
            "/freshness/trends",
            "/reports",
            "/remediation-authority",
        ),
        allowed_dependency_categories=("auth", "tenant", "db", "rate"),
        required_make_targets=("audit-chain-verify",),
        required_ci_gates=COMMON_GATES,
        maturity_tag="production-grade",
        required_route_invariants=("auth", "tenant_bound", "scoped"),
        auth_class=AuthClass(
            required_scope_prefixes=(
                "audit:",
                "attestation:",
                "forensics:",
                "admin:",
                "compliance:",
                "remediation:",
            ),
            require_any_scope=True,
            tenant_binding_required=True,
        ),
        global_routes=(
            ex(
                "POST",
                "/approvals",
                "global_admin",
                "Approval creation operates at platform governance scope.",
            ),
            ex(
                "POST",
                "/approvals/verify",
                "global_admin",
                "Approval verification operates at platform governance scope.",
            ),
            ex(
                "GET",
                "/approvals/{subject_type}/{subject_id}",
                "global_admin",
                "Approval lookup operates at platform governance scope.",
            ),
            ex(
                "POST",
                "/evidence/bundles",
                "global_admin",
                "Evidence bundles can be globally exported.",
            ),
            ex(
                "GET",
                "/evidence/bundles/{bundle_id}",
                "global_admin",
                "Evidence bundle retrieval can be global.",
            ),
            ex(
                "POST",
                "/evidence/verify",
                "global_admin",
                "Evidence verification can be global.",
            ),
        ),
        public_routes=(
            ex(
                "GET",
                "/reports/health",
                "public",
                "Unauthenticated Report Authority health probe.",
            ),
            ex(
                "GET",
                "/remediation-authority/health",
                "public",
                "PR 18.3 Remediation Authority health probe — no auth, no tenant data.",
            ),
        ),
    ),
    PlaneDef(
        plane_id="workforce",
        route_prefixes=("/workforce",),
        allowed_dependency_categories=("auth", "tenant", "db", "rate"),
        required_make_targets=("soc-invariants",),
        required_ci_gates=COMMON_GATES,
        maturity_tag="production-grade",
        required_route_invariants=("auth", "tenant_bound"),
        auth_class=AuthClass(
            required_scope_prefixes=("admin:",),
            require_any_scope=True,
            tenant_binding_required=True,
        ),
        public_routes=(
            ex(
                "POST",
                "/workforce/users/accept-invite",
                "auth_exempt",
                "Invite-token exchange: the invite token IS the credential. "
                "32-byte URL-safe random, 72h TTL, single-use (cleared on acceptance). "
                "Tenant isolation enforced implicitly: DB lookup returns nothing for "
                "tokens not belonging to the caller's tenant. Role assigned by the "
                "inviting admin at creation time; accepting user cannot escalate.",
                permanent=True,
            ),
        ),
    ),
    PlaneDef(
        plane_id="ui",
        route_prefixes=("/ui",),
        allowed_dependency_categories=("auth", "tenant", "rate"),
        required_make_targets=("test-quality-gate",),
        required_ci_gates=COMMON_GATES,
        maturity_tag="production-grade",
        required_route_invariants=("auth", "tenant_bound"),
        auth_class=AuthClass(
            required_scope_prefixes=(
                "ui:",
                "admin:",
                "audit:",
                "controls:",
                "forensics:",
            ),
            require_any_scope=True,
            tenant_binding_required=True,
        ),
        bootstrap_routes=(
            ex(
                "GET",
                "/ui/csrf",
                "bootstrap",
                "CSRF bootstrap endpoint before tenant-bound session materializes.",
            ),
            ex(
                "GET",
                "/ui/scopes",
                "bootstrap",
                "Scope introspection endpoint before tenant-bound flow.",
            ),
            ex(
                "GET",
                "/ui/token",
                "bootstrap",
                "Token bootstrap endpoint before tenant-bound flow.",
            ),
            ex(
                "GET",
                "/ui/entitlements/registry",
                "bootstrap",
                "Capability registry: static list of all capabilities/namespaces. Public for feature-discovery pre-auth.",
            ),
        ),
        global_routes=(
            ex("GET", "/ui/ai", "global_admin", "Global AI dashboard overview."),
            ex(
                "GET",
                "/ui/controls",
                "global_admin",
                "Global controls dashboard overview.",
            ),
            ex(
                "GET",
                "/ui/controls/{inv_id}",
                "global_admin",
                "Global control detail view.",
            ),
            ex(
                "GET",
                "/ui/dash/controls",
                "global_admin",
                "Global controls dashboard rollup.",
            ),
            ex(
                "GET",
                "/ui/dash/forensics",
                "global_admin",
                "Global forensics dashboard rollup.",
            ),
            ex(
                "GET",
                "/ui/dash/posture",
                "global_admin",
                "Global posture dashboard rollup.",
            ),
            ex("GET", "/ui/feed", "global_admin", "Global UI feed endpoint."),
            ex(
                "POST",
                "/ui/audit/packet",
                "global_admin",
                "Global audit packet creation.",
            ),
            ex(
                "GET",
                "/ui/audit/packet/{packet_id}/download",
                "global_admin",
                "Global audit packet download.",
            ),
        ),
    ),
    # P0-11 CGCT routes live under /control-tower, already owned by the
    # control plane above. No separate PlaneDef required.
    PlaneDef(
        plane_id="identity",
        route_prefixes=("/identity",),
        allowed_dependency_categories=("auth", "tenant"),
        required_make_targets=("soc-invariants",),
        required_ci_gates=COMMON_GATES,
        maturity_tag="tester-ready",
        auth_class=AuthClass(
            required_scope_prefixes=("identity:",),
            require_any_scope=True,
            tenant_binding_required=False,
        ),
        public_routes=(
            ex(
                "GET",
                "/identity/invitations/{token}",
                "public",
                "P-113.8 invitation preflight: public endpoint; fgwi1.* token is "
                "the credential. Returns masked display info only — no tenant_id, "
                "no internal IDs, no PII beyond masked email.",
                permanent=True,
            ),
        ),
        auth_exempt_routes=(
            ex(
                "POST",
                "/identity/invitations/accept",
                "auth_exempt",
                "Invite-token exchange: the invite token IS the credential. "
                "SHA-256 hash comparison enforced; single-use replay protection. "
                "No prior auth credential exists for a new invitee.",
                permanent=True,
            ),
            ex(
                "POST",
                "/identity/invitations/{token}/accept",
                "auth_exempt",
                "P-113.8 canonical invitation acceptance: uses platform-admin gateway "
                "auth (require_internal_admin_gateway) + fgwi1.* HMAC token authority. "
                "Not gated by identity: scope — authority is the locked invitation "
                "record + gateway credential.",
                permanent=True,
            ),
        ),
    ),
]


def list_planes() -> list[dict[str, object]]:
    return [p.to_dict() for p in PLANE_REGISTRY]
