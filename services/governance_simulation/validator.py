...
def validate_simulation(
    ...
    *,
    ...
    derived_entity_ids = (
        {e.id for e in derived_entities} if derived_entities is not None else snapshot_entity_ids
    )

    # missing_source_snapshot (FATAL)
    if not overlay.source_snapshot_id:
        violations.append("missing_source_snapshot")
        findings.append(SimulationValidationFinding(
    ...
    add_ops = {
        OverlayOperationType.add_entity.value,
        OverlayOperationType.add_relationship.value,
    }
    remove_entity_ops = {OverlayOperationType.remove_entity.value}
    modify_entity_ops = {OverlayOperationType.modify_entity.value}
    remove_rel_ops = {OverlayOperationType.remove_relationship.value}
    modify_rel_ops = {OverlayOperationType.modify_relationship.value}
    ...
    for op in overlay.operations:
        ...
        if op_type == OverlayOperationType.add_relationship.value and op.relationship_payload:
    ...
    if scenario_category is not None and scenario_category not in SCENARIO_CATEGORY_REGISTRY:
