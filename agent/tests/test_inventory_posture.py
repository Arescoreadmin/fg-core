...
def test_inventory_and_posture_shapes():
    ...
    pos = collect_posture()
    assert {"platform", "os_version", "agent_version", "device_id", "capabilities"}.issubset(inv.keys())
    assert {"os_version", "root_or_jailbreak_signals", "compliance_status"}.issubset(pos.keys())
