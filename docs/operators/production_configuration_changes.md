...
|---|---|---|---|
| `CORE_TENANT_ID` | `frostgate-internal` | Railway `api` service, Vercel `apps/console`, Vercel `apps/portal` | `api/internal_platform_authority.py` (backend, hard-fail); `apps/console/lib/startup-validation.ts` (console, hard-fail at startup); `apps/portal/app/api/core/[...path]/route.ts` (portal, fails per-request if unset — does not reject `default` specifically) |
