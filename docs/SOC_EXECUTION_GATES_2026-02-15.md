...
- `.github/workflows/testing-module.yml` — `fg-fast` job `timeout-minutes` raised 15→35; `fg-security` job raised 15→25. Root cause: `fg-fast-pytest hard_max` was raised from 900s to 930s in the budget-stability fix; `make fg-fast` total (pre-checks + pytest + lint) takes ~20 min in CI, and the 5 post-steps (required-tests-gate, policy-drift-check, budget-enforcement, security-invariant-coverage, lane-runner) add ~5 min more — requiring a 35-min ceiling to include setup overhead. `fg-security` (`make fg-security`: policy-validate + soc-invariants + ~701 security tests) takes ~20 min in CI, requiring a 25-min ceiling.
