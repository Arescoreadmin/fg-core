...
- `tools/ci/check_authority_integration.py` — Check 7 upgraded from `warnings.append` to `errors.append`; now causes non-zero exit on any authority with `cgin_snapshot: true` and `cgin_anonymized: false`.
- `tests/test_cgin_privacy.py` — 140 new tests covering fingerprint determinism, uniqueness, irreversibility, `assert_snapshot_safe`, all 6 fixed authorities, schema validation, already-correct authorities.
