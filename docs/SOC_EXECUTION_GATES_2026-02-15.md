...
**Change scope:** All four files are auto-generated deterministic artifacts regenerated by `make route-inventory-generate` and the contract toolchain. They reflect the addition of five new internal-admin routes for the canonical platform service principal introduced in PR #586. No CI workflow, OPA policy, auth middleware, or cryptographic key handling code changed.

**New routes registered:**

| Method | Path | Scope required | Plane |
|--------|------|---------------|-------|
| GET | `/admin/system/service-principal` | `admin:read` | control / global_admin |
| POST | `/admin/system/service-principal/rotate` | `admin:write` | control / global_admin |
| POST | `/admin/system/service-principal/suspend` | `admin:write` | control / global_admin |
| POST | `/admin/system/service-principal/resume` | `admin:write` | control / global_admin |
| POST | `/admin/system/service-principal/revoke` | `admin:write` | control / global_admin |

All five routes are: (a) protected by `require_internal_admin_gateway` (X-FG-Internal-Token HMAC), (b) protected by `require_scopes("admin:read"/"admin:write")`, (c) guarded by `require_permission("platform.admin")`, and (d) registered as `global_admin` exceptions in the plane registry (no customer tenant context). These routes expose no customer PII, no credential secrets, and return safe metadata only.

**Security invariants enforced by this change:**

1. **Machine identity is explicit and constrained.** The PSP is a named, persisted, non-human machine identity. It binds only to an active `internal_platform` tenant. Customer, demo, validation, and unknown kinds are rejected at bootstrap and at every authentication check.
2. **No global superuser.** The PSP is granted a small explicit set of platform permissions (`platform.authority.verify`, `platform.tenants.read`, `platform.tenants.operate`, `platform.credentials.rotate`, `platform.audit.write`). `platform.admin` is explicitly excluded. Target tenant authorization, RBAC, and RLS all remain in force.
3. **Revocation is irreversible.** A revoked PSP cannot authenticate, cannot rotate its credential, and cannot be suspended or resumed. Bootstrap creates a new identity.
4. **No raw secret in any API response, log, or audit event.** GET returns safe metadata only. Audit events carry credential_id and generation but never secret material. This is enforced by `safe_dict()` and the `emit_service_principal_event()` function.
5. **Append-only audit table.** `platform_service_principal_events` has an BEFORE UPDATE OR DELETE trigger that raises `restrict_violation`, identical to the pattern in `internal_platform_authority_events` (PR #585).
6. **Duplicate principal impossible.** A `UNIQUE INDEX uq_psp_stable_key` on `stable_key` and a partial unique index `uq_psp_authority_kind_active` on `(authority_tenant_id, principal_kind) WHERE lifecycle_state != 'revoked'` serialize concurrent bootstrap attempts at the DB level. Application-level idempotency key in `CredentialAuthority.issue_credential()` prevents duplicate credential issuance.

**Non-changes (audited, confirmed correct):**

- No existing customer credential path changes. Console, Portal, and customer API-key flows are untouched.
- No `api/auth`, `api/middleware`, `admin_gateway/auth`, or `policy/opa` files modified.
- No production startup validation changes. PSP validation is non-blocking in non-strict mode.
- RBAC role permissions unchanged. `platform_admin` role inherits the 5 new PSP permissions via `ALL_PERMISSIONS` (as designed — `platform_admin = ALL_PERMISSIONS`); no other role gains them.
- `ActorContext` gains two optional fields (`service_principal_id`, `authority_tenant_id`) that default to `None` for all existing flows. PR #587 will populate them for machine-identity callers.

**Tests added:** 54 tests in `tests/test_platform_service_principal.py` covering bootstrap, authority binding, lifecycle, credential, authentication, authorization, audit, security invariants, and migration documentation.

**SOC review outcome:** approved. PR #586 adds machine identity as an explicit, constrained, independently revocable layer on top of PR #585's operator authority. The five new routes are operator-internal only (X-FG-Internal-Token gated), return no secrets, and add no customer-visible surface. The PSP is not a superuser, cannot bypass RLS, and cannot be used as a customer credential. All changed `tools/ci/*.json` and `.sha256` files are deterministic regenerations of route and topology state, not policy changes.
