...
| Error reporting | `sentry-sdk[fastapi]` installed and initialized in `api/main.py`; **no evidence of a startup-time assertion that `SENTRY_DSN` is set**, unlike the console's `instrumentation.ts` → `validateProductionConfig()` pattern | **Two-step recommendation:** (1) close the configuration gap first — add a startup check mirroring the console's fail-fast pattern so Sentry either has a real DSN in prod or the deploy fails loudly, not silently degrades. (2) Only after (1) is confirmed, a read-only Sentry issues/events connector scoped to the FrostGate prod project becomes genuinely useful for fast triage. | Read-only, prod project scoped, contingent on (1) | Medium — prod error events can carry tenant-identifying context in stack traces/breadcrumbs; confirm PII scrubbing rules in the Sentry project config before any connector is wired, since regulated-industry clients (healthcare per SYSTEM.md) may have error payloads containing sensitive identifiers |
