...
## PR 18.6.3 Decisions — Operations Workspace (2026-07-03)

| Decision | Rationale | Impact |
| --- | --- | --- |
| InvestigationDrawer as separate component (not part of WidgetShell) | Prevents coupling between the metadata shell and investigation concerns. Not every widget needs investigation support — optional opt-in is cleaner than shell-level feature. `InvestigationDrawer` receives authority metadata from its caller, not from WidgetShell. | InvestigationDrawer is a standalone `role="complementary"` panel. WidgetShell gains optional `investigationSupport` prop to indicate widget-level support. No breaking changes to existing widgets. |
| CorrelationGraph as list-based rendering (not canvas, not SVG) | Accessibility requires keyboard navigability and screen reader support. Canvas and SVG graph layouts introduce randomness and require complex accessibility layers. List-based rendering is deterministic, accessible, and testable statically. | `role="list"` on node container. Nodes sorted by type then id for determinism. No canvas, no SVG, no layout algorithms. |
| ReplaySeam as disabled seam — no placeholder data | Capability gates the future replay feature without leaking fake data. All buttons `disabled` with `aria-disabled="true"`. Shows informational banner "Replay not available from current authority data". No mock timelines, no fabricated snapshots. | 6 disabled replay control buttons. Banner text is authority-accurate. No behavior. |
| FutureReservedPanels isolated — no primary experience contamination | Future capabilities should not appear in primary navigation or affect any current user journey. Isolated in a dedicated section below existing widgets. All 10 panels explicitly disabled. "Capability reserved" text prevents misinterpretation. | No nav items. No active interactions. No contamination of existing widget behavior. |
| OperationalHealthMatrix derives from snapshot — no fabricated health data | Health status must reflect real snapshot data. `billing` and `identity` rows show `unknown` because they are not in the control tower snapshot. `navigation` row shows `ok` because the registry is always loaded. No uptime percentages are hardcoded. | Matrix derives from `ControlTowerSnapshotV1` fields: `chain_integrity.status`, `agents.quarantine_count`, `connectors.errors.length`, `keys.expired`, `lockers.locked_count`. Null snapshot renders all rows as `unknown`. |
