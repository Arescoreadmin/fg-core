...
1. `api/auth_scopes/resolution.py` — Added `_lookup_canonical_platform_admin_role(engine, tenant_id, credential_id)` private helper. Unconditionally queries `tenant_credential_roles` via `get_credential_role()` after canonical credential authentication. Sets `app.tenant_id` on PostgreSQL connections so RLS policy on `tenant_credential_roles` resolves the row. Returns `True` only if `role_name == 'platform_admin'` and `revoked_at IS NULL`. Fail-closed: any exception returns `False`. RBAC lookup now runs BEFORE scope enforcement — `canonical_platform_admin` credentials (may have empty scopes) bypass legacy scope check since `platform.admin` subsumes all scope requirements. Updated `bind_tenant_id()` at three locations to include `"canonical_platform_admin"` alongside `"admin_internal_token"` in the delegated-tenant branch.
2. `api/middleware/auth_gate.py` — Added `result.reason != "canonical_platform_admin"` condition to the tenant mismatch early-rejection (lines 166–187). Canonical platform-admin credentials carry `tenant_id="frostgate-internal"` but operate on customer tenants via the verified delegated-tenant path in `bind_tenant_id()`. Without this deferral, the middleware rejects the X-Tenant-ID mismatch before delegation proof can run — same pattern and intent as the `admin_internal_token` exclusion (PR-CORE-002B). The `_tenant_injectable` guard is not changed; `canonical_platform_admin` is already excluded (it is not `global_key`).
3. `api/identity_providers/api_key.py` — Extended named-user delegation block to handle `canonical_platform_admin`. Legacy (`admin_internal_token`) path: permissions from scope strings (unchanged). Canonical path: permissions from `roles_to_permissions(["platform_admin"])` directly (RBAC-proven, no scope derivation).
4. `api/entitlements.py` — Extended `_ensure_admin_gateway_tenant_bound()` and `_admin_gateway_tenant_admin_satisfies_capability()` to also handle `canonical_platform_admin` reason, applying the same delegation proof + tenant lifecycle verification path as `admin_internal_token`.
...
**Critical-path files changed:**
- `api/admin_identity.py`: one new `GET` route + `EngagementSelectorItem` + `EngagementSelectorList` models + imports (`FaEngagement`, `list_engagements`, `func`, `select`, `Query`). Response carries `items`, `next_cursor`, `total_count`. No existing routes modified.
- `contracts/core/openapi.json`, `schemas/api/openapi.json`: new path + `EngagementSelectorList` schema entry.
- `BLUEPRINT_STAGED.md`, `CONTRACT.md`: contract authority SHA updated.
- `tools/ci/route_inventory.json`, `tools/ci/route_inventory_summary.json`, `tools/ci/plane_registry_snapshot.json`, `tools/ci/topology.sha256`: regenerated via `make route-inventory-generate` (topology hash changes because new response schema added).

**Tests:** 6 integration tests via `build_app`/`TestClient` — own-tenant 200+data+total_count, cross-tenant 403, empty-state `{items:[],total_count:0,next_cursor:null}`, server-side isolation, wrong-scope 403, cursor pagination (2-page walk with distinct IDs + consistent total_count).
...
**Changes:**
- `api/auth_dispatch.py` — `_try_jwt_actor()` extended with FIAP code path at the top of the function. When `FG_IDENTITY_AUTHORITY_ENABLED=1`, routes through `get_identity_authority().authenticate_jwt()` and converts result to `ActorContext` via `.to_actor_context()`. When flag is off (default), execution falls through to the existing Auth0-only block unchanged. All existing callers of `get_actor_context()` and `require_permission()` are unaffected.
- `api/identity_authority/__init__.py` — package public exports: `IdentityAuthority`, `get_identity_authority`, `get_authorization_context`, `require_permission_v2`, `get_actor_context_compat`, `AuthorizationContext`, `CanonicalIdentity`, `TenantBinding`, error types.
- `api/identity_authority/models.py` — frozen dataclasses: `IdentityProvider`, `AuthenticationContext`, `TenantBinding`, `IdentitySubscription`, `CanonicalIdentity`, `AuthorizationContext`, `IdentityClaims`. `CanonicalIdentity.to_actor_context()` bridges to legacy `ActorContext`.
- `api/identity_authority/authority.py` — `IdentityAuthority` singleton. `authenticate_jwt()`: provider registry → `CanonicalIdentity` → `TenantResolver` → `AuthorizationContext` → audit + metrics. `authenticate_api_key()`: delegates to `MachineIdentityAuthority`. `create_session()`, `logout()`, `logout_all()`.
- `api/identity_authority/providers/` — `base.py` (`IdentityProviderProtocol`, `IdentityValidationError`, `IdentityProviderError`); `registry.py` (`IdentityProviderRegistry` — ordered chain Auth0 → Entra → Google → Generic OIDC from env config); `auth0_provider.py`, `entra_provider.py`, `google_provider.py`, `generic_oidc_provider.py` — all implement `IdentityProviderProtocol`. Provider resolution: first configured provider that accepts the token wins; `IdentityProviderError` (provider down) propagates immediately without trying next provider.
- `api/identity_authority/providers/entra_provider.py` — full Entra ID implementation. Two-pass decode: unverified extract of `tid`, then RS256/ES256 verify with per-tenant JWKS URL and issuer. Multi-tenant: validates `tid` against `FG_ENTRA_ALLOWED_TENANTS`. MFA detection from AMR: `{"mfa","ngcmfa","fido2","fido","rsa"}`. Subject: prefers `oid` over `sub`. Email: `email` → `preferred_username` → `upn` fallback.
- `api/identity_authority/session_authority.py` — `SessionAuthority`: HMAC-SHA256 signed sessions (`FG_SESSION_SECRET`). Token format: `base64url(json).hmac_sha256`. TTL: 8h absolute + 1h idle + 30min refresh window. `_RevocationStore`: Redis-backed (`fg:session:revoked:{sid}` with TTL) with thread-safe in-memory dict fallback. `SessionExpiredError`, `SessionRevokedError`, `SessionInvalidError` are distinct exception types.
- `api/identity_authority/tenant_resolver.py` — `TenantResolver`: queries `TenantUser` by identity triple `(identity_provider, identity_issuer, identity_subject, binding_status="bound", active=True)`. Cross-tenant escalation prevention in `_resolve_by_hint`: if identity has an existing `tenant_binding`, the hint must exactly match its `tenant_id` or is denied (warning + audit event). Gracefully returns `None` if `admin_gateway` is unavailable.
- `api/identity_authority/machine_identity.py` — `MachineIdentityAuthority`. Primary path: `authenticate_api_key_from_state()` reads `request.state.auth` (set by existing API key middleware) and wraps it in `CanonicalIdentity` with `identity_type="machine"` — no DB round-trip. Secondary path: `authenticate_api_key()` loads `TenantApiKey` from DB, checks active flag, calls `roles_to_permissions()`.
- `api/identity_authority/migration.py` — `LegacySessionMigrator`. Validates portal HMAC tokens (`PORTAL_PASSWORD`) and admin_gateway tokens (`FG_SESSION_SECRET`/`ADMIN_SESSION_SECRET`). Raises `LegacyMigrationError` on expired or unknown-format tokens. `build_identity_from_legacy()` produces `CanonicalIdentity` with `identity_type="human"` and `provider.name="portal_legacy"`.
- `api/identity_authority/portal_identity.py` — `PortalIdentityBridge`. `validate_portal_session()` tries unified session first, then legacy migration. `validate_shared_secret()` constant-time compares against `PORTAL_PASSWORD` (deprecated path).
- `api/identity_authority/audit.py` — `IdentityAuditor`: SHA-256 hash chain `H(prev_hash || event_id || ts || event_type || subject || tenant_id || details)`. 25 `IdentityEventType` values. Logs structured JSON always; persists to `SecurityAuditLog` when DB factory available. `_sanitize()` strips `token`, `secret`, `password`, `key`, `access_token`, `refresh_token`, `id_token`, `client_secret`, `authorization`, `cookie` from detail dicts.
- `api/identity_authority/metrics.py` — Prometheus metrics: `frostgate_identity_auth_success_total`, `frostgate_identity_auth_failed_total`, `frostgate_identity_auth_latency_seconds`, `frostgate_identity_active_sessions_total`, `frostgate_identity_session_created_total`, `frostgate_identity_session_revoked_total`, `frostgate_identity_tenant_resolution_seconds`, `frostgate_identity_tenant_resolution_total`, `frostgate_identity_legacy_migration_total`.
- `api/identity_authority/integration.py` — FastAPI dependency layer. `get_authorization_context()`: dev bypass → Bearer JWT → API key state → anonymous. `require_permission_v2(*perms)`: enforces ALL listed permissions, returns `AuthorizationContext`. `get_actor_context_compat()`: shim returning `ActorContext` for routes not yet migrated.
- `api/identity_providers/entra.py` — `EntraProvider.extract_actor()` now delegates to `EntraOIDCProvider` instead of `raise NotImplementedError`. Preserved as thin shim for backwards import compatibility.
- `tests/identity_authority/` — 64 tests: 8 files covering session lifecycle, migration, tenant resolution, provider registry, authority integration, machine identity, Entra provider.
- `docs/identity/ARCHITECTURE.md` — component map, auth flow diagrams, model tables, provider chain, session config, audit chain hash function, feature flags.

**Security posture:**
- **Flag containment:** `FG_IDENTITY_AUTHORITY_ENABLED` defaults to `0`. The FIAP code path in `auth_dispatch.py` is entirely gated behind this flag. The production default behavior is the existing Auth0-only path. No operator can accidentally enable FIAP.
- **Auth boundary:** `IdentityAuthority` is the single entry point for all authentication when FIAP is enabled. No code outside `api/identity_authority/` performs authentication.
- **Cross-tenant isolation:** `TenantResolver._resolve_by_hint()` explicitly validates that any tenant ID hint matches the identity's already-bound tenant. A mismatch results in denial + warning log + audit event. Machine identities (API keys) may accept hints directly, consistent with their pre-existing behavior via `request.state.auth`.
- **Session security:** HMAC-SHA256 signed tokens with `FG_SESSION_SECRET`; constant-time comparison on all signature checks (`hmac.compare_digest`). Revocation enforced on every request via Redis or in-memory store. Old session revoked before new session issued on refresh (replay prevention).
- **Entra MFA enforcement:** `FG_ENTRA_REQUIRE_MFA=1` rejects Entra ID tokens that do not contain a recognized MFA AMR value. Default is off to preserve login continuity during rollout.
- **Entra multi-tenant security:** `FG_ENTRA_ALLOWED_TENANTS` whitelist enforced per-token using the `tid` claim. An Entra token from an unlisted tenant is rejected with `TENANT_NOT_ALLOWED`.
- **Audit chain integrity:** SHA-256 chain ensures tamper-evidence across the process lifetime. `_sanitize()` prevents secrets from appearing in audit records.
- **No new routes:** this PR adds zero new API routes. The `api/identity_authority/` package is a service module, not a router.
- **No new DB migrations:** no schema changes. `TenantUser` and `TenantApiKey` reads are additive queries against existing tables; both are wrapped in `try/except ImportError` + `try/except Exception` so the authority degrades gracefully if `admin_gateway` is not available.
- **No middleware changes:** existing API key middleware continues to set `request.state.auth` exactly as before. FIAP reads this state but does not modify middleware behavior.
- **No OPA / policy changes.**
- **No guard weakened:** `api/auth_dispatch.py` remains classified under `api/auth` CRITICAL_PREFIX in `check_soc_review_sync.py`. No `NO_FIX_LOG_REQUIRED` marker used.

**Validation:**
- `.venv/bin/pytest tests/identity_authority/ -q`: 64 passed, 0 failed
- `make soc-manifest-verify`: OK (all P0 findings mitigated)
- `make soc-review-sync` (local): OK after this doc update
- `python tools/ci/guard_pr_fix_log.py --base origin/main --head HEAD`: OK after `docs/ai/PR_FIX_LOG.md` update

**SOC review outcome:** approved with noted open items. Auth dispatch change is entirely flag-gated (`FG_IDENTITY_AUTHORITY_ENABLED=0` is the default and production state). Entra ID provider replaces a `NotImplementedError` stub with a full RS256/ES256 + JWKS implementation with multi-tenant and MFA enforcement. Session authority matches the existing HMAC-SHA256 pattern already in use in `admin_gateway`. Tenant resolver enforces cross-tenant isolation at the authority layer in addition to the existing per-route `require_bound_tenant()` guard. No routes, middleware, migrations, OPA policies, or CI guards changed. **Open items:** (1) `_verify_secret()` in `machine_identity.py` is a stub for the direct API key authentication path — HMAC check against `hashed_secret` must be implemented before any direct-path caller is wired. (2) Portal Next.js layer must be updated to use `PortalIdentityBridge` before the `PORTAL_PASSWORD` shared-secret model can be deprecated. (3) `FG_IDENTITY_AUTHORITY_ENABLED` must not be enabled in production until the provider chain is validated end-to-end in a staging environment.
