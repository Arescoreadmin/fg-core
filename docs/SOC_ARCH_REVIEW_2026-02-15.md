...
### Files changed (SOC-critical)
- `.github/actions/fg-secrets/action.yml`

### Why
- `env/prod.env` did not exist — `docker-compose.yml` requires it as an explicit `env_file`; `docker-validate` was failing with "env file not found". Created as a comment-only placeholder (no secrets).
- `NATS_AUTH_TOKEN` was not generated by `.github/actions/fg-secrets/action.yml` — `prod-profile-check` calls `docker compose config` which uses `${NATS_AUTH_TOKEN:?...}` hard interpolation, crashing Guard and fg-required jobs.

### Risk
- **fg-secrets change:** SOC-critical because it modifies CI secret generation. Risk is low: NATS_AUTH_TOKEN is a service-to-service bus credential (not a human auth path), the generated value is ephemeral and random, and the pattern matches docker-ci.yml.
- **env/prod.env:** Not SOC-critical (no auth logic). Included for completeness.

### Validation performed
- `make soc-review-sync` — PASSED
- `make fg-contract` — PASSED
- `make test-quality-gate` — PASSED

### Rollback
- Remove NATS_AUTH_TOKEN line from fg-secrets action and delete env/prod.env.

<!-- SOC-CI-FIX-2026-03-01::nats-auth-token-env-prod -->
