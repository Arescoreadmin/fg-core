...
## [2026-03-01] Fix Console `@/*` Path Alias Resolution in Build Environments

### Summary
Added explicit TypeScript `baseUrl` in `console/tsconfig.json` so `@/*` path aliases resolve reliably across CI/build environments and containerized Next.js builds.

### Symptom
Console builds failed with module resolution errors for aliased imports such as `@/lib/api`, `@/lib/coreApi`, `@/styles/tokens`, and `@/components/tables/DecisionsTable`.

### Root Cause
`console/tsconfig.json` declared `paths` alias mapping for `@/*` but did not define `baseUrl`, which can cause alias resolution drift across tooling/runtime contexts.

### Impact Surface
- Files: `console/tsconfig.json`
- Services: Console build pipeline
- Profiles: CI/container build contexts
- Governance surfaces affected: deterministic build reproducibility

### Resolution
Set `"baseUrl": "."` under `compilerOptions` in `console/tsconfig.json`, preserving existing alias mapping without introducing stub modules or fake exports.

### Gates Executed
- `cd console && npm run build`
- `make fg-fast` (existing known environment limitation remains in `ci-admin` lane due offline dependency fetch)

### Final Status
PASS (for console alias fix); environment warning for unrelated `ci-admin` network restriction.
