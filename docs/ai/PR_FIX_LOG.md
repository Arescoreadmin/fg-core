...
### 2026-06-03 — PR 3: External AI Risk Register (12th scan type + Governance Intelligence Addendum)

**Branch:** `pr/3-external-ai-risk-register`

**PR/context:** PR 3 — External AI Risk Register — deterministic risk scoring and governance intelligence layer on top of PR 1 (AI Tool Discovery) and PR 2 (AI Data Access Mapping)

**Area:** Field Assessment / Connector Layer / Console UI / Portal UI / Evidence Pipeline / Verification Bundle

**Summary of changes:**

PR 3 adds `external_ai_risk_register` as the 12th scan type. It is a `provider: passive` connector that reads PR 1 and PR 2 scan results and produces:
- Deterministic risk records (one per AI tool per engagement) with 8 risk categories and 4 score bands (low/moderate/high/critical)
- Governance Intelligence Addendum (15 additions): governance state, regulatory impact flags, risk ownership model, vendor governance status, decision linkage, risk aging, remediation tracking, executive dashboard metrics, evidence graph preparation
- 3 API routes: POST run, GET list, PATCH review
- Full idempotency chain: stable_ts from PR1 collected_at → evidence_hash → create_scan_result dedup → create_finding dedup
- Report section integration and verification bundle extension

**Governance addendum (25 new ORM columns, migration 0091):**

1. Risk Ownership Model: `risk_owner`, `owner_type` (9 values including IT/Security/Legal/Finance/HR/Compliance/Operations/Product/Unknown)
2. Governance State: `governance_state` (ungoverned/partially_governed/governed/exception_granted/unknown) — deterministic at generation, exception_granted preserved across re-scans
3. Decision Linkage: `decision_refs`, `risk_acceptance_refs`, `exception_refs`, `approval_refs` (JSON arrays, updatable via PATCH)
4. Vendor Governance Status: `vendor_review_status`, `vendor_dpa_status`, `vendor_baa_status`, `vendor_security_review_status`, `vendor_last_reviewed_at`
5. Regulatory Impact Flags: `regulatory_flags` — 10 frameworks deterministic (NIST_AI_RMF always; EU_AI_ACT for unverified/shadow; GDPR+State_Privacy_Law for sensitive; ISO_42001 for governance gaps; HIPAA/PCI_DSS/SOX/GLBA/FFIEC from exposure keywords)
6. Risk Aging: `risk_age_days`, `first_detected_at`, `last_observed_at`, `last_reviewed_at`
7. Remediation Tracking: `remediation_status`, `remediation_target_date`, `remediation_completed_at`
8. Executive Dashboard: `build_summary()` extended with governance_dist, vendor_dist, remediation_dist, regulatory_dist, risks_without_review, risks_without_vendor_approval, stale_risks
9. Report Enhancements: all new fields surfaced in report section
10. Evidence Graph Prep: `graph_node_id`, `risk_node_id`, `owner_node_id`, `vendor_node_id`, `decision_node_id`, `governance_node_id`

**High-risk files changed:**

- `api/db_models_external_ai_risk.py` — NEW: `FaExternalAiRiskRecord` ORM model (55 columns, 5 indexes, unique constraint on engagement_id+tenant_id+tool_name)
- `api/field_assessment.py` — 3 new routes (POST run, GET list, PATCH review); `ExternalAiRiskRecordResponse` (55 fields); `ExternalAiRiskReviewUpdateRequest` with `extra="forbid"` enforcing immutability; validation constants `_VALID_GOVERNANCE_STATES`, `_VALID_REMEDIATION_STATUSES`, `_VALID_OWNER_TYPES`
- `migrations/postgres/0090_external_ai_risk_register.sql` — NEW: initial table + indexes
- `migrations/postgres/0091_external_ai_risk_register_addendum.sql` — NEW: 25 addendum columns + 2 indexes (all ADD COLUMN IF NOT EXISTS, safe for replay)
- `services/connectors/external_ai_risk_register/risk_engine.py` — NEW: deterministic risk engine (categories, scoring, governance state, regulatory flags, build_summary)
- `services/field_assessment/connectors/external_ai_risk_register_bridge.py` — NEW: H12/H13/H15 bridge; upsert with exception_granted preservation; aging computation
- `services/field_assessment/models.py` — `EXTERNAL_AI_RISK_REGISTER` enum value added
- `services/field_assessment/scan_registry.py` — schema version + required fields entries
- `services/verification_bundle/bundle_service.py` — extended ai_risk_data list comprehension to include all 25 addendum fields
- `apps/console/components/field-assessment/ExternalAiRiskRegisterPanel.tsx` — NEW: full console panel with executive metrics, governance filter, per-record governance/age/regulatory/owner/vendor badges
- `apps/portal/app/engagement/[engagementId]/page.tsx` — portal engagement page extended with AI risk section
- `tools/ci/contract_routes.json` — regenerated (3 new routes)
- `tools/ci/plane_registry_snapshot.json` — regenerated
- `tools/ci/route_inventory.json` — regenerated
- `tools/ci/route_inventory_summary.json` — regenerated
- `tools/ci/topology.sha256` — regenerated
- `BLUEPRINT_STAGED.md` — contract authority SHA updated after Pydantic model expansion
- `docs/SOC_ARCH_REVIEW_2026-02-15.md` — PR 3 and addendum entries added (required by soc-review-sync gate)

**Security posture:**

No change to authentication or authorization model. All 3 routes are tenant-scoped via `require_bound_tenant`. PATCH route enforces `extra="forbid"` — any attempt to mutate immutable fields (risk_score, risk_reason, regulatory_flags, etc.) returns HTTP 422 at the Pydantic layer. No new MS Graph scopes — `provider: passive` makes zero external network calls. All data derived from evidence already collected.

**Audit posture:**

All 3 routes call `_c6_write_audit_event` directly for scan.initiated, scan.completed, scan.failed, ai_risk_record.review_updated events. Satisfies H13.5 AST coverage enforcement without audit_exceptions.yaml entries. Confirmed: `python tools/ci/check_audit_coverage.py` passes at 100% coverage.
...
- H13/H13.5: `_c6_write_audit_event` direct call in route body (AST-detectable)
- H15: `FaScanResult` auto-enters `collected` lifecycle state
- PR 52/52.5: verification bundle captures all 55 columns including all 25 addendum fields

**Idempotency:** `stable_ts = pr1_scan.collected_at` → `compute_evidence_hash(raw_payload)` → `create_scan_result` dedup on `(engagement_id, tenant_id, evidence_hash)`. Bridge preserves existing record IDs via `existing_ids` from DB. `create_finding` dedup on `findings_hash = SHA256(finding_type|engagement_id|source_ref)`. `exception_granted` governance state preserved across re-scans. Second run creates no new rows.

**Tests/gates run:**

- `pytest tests/test_external_ai_risk_register.py` — 126/126 passed (81 original + 43 addendum + 2 hardening)
- `make fg-fast` — PASS (all gates green, exit 0)
- `bash codex_gates.sh` — ruff lint: PASS, ruff format: PASS, mypy: PASS
- `python tools/ci/check_audit_coverage.py` — PASS (100% coverage, 0 violations)
- `make route-inventory-audit` — PASS (3 routes registered)
- `python tools/ci/check_plane_registry.py` — 3 pre-existing failures from PR 52.5 portal routes (not PR 3)

**Known limitations:**

- `vendor_dpa_status`, `vendor_baa_status`, `vendor_security_review_status` always default to "unknown" at generation; full vendor governance enrichment is planned for PR 3.5
- `exception_granted` governance state can only be set by operators via PATCH; no automated exception workflow yet
- `remediation_target_date` and `remediation_completed_at` are free-form ISO strings; no date validation enforced at API layer
- Graph node IDs (`risk_node_id`, `owner_node_id`, etc.) are generated with deterministic patterns but the graph layer itself is planned for a future PR

**Follow-up work:**

- PR 3.5 (planned): vendor governance enrichment — DPA/BAA/security review status from vendor metadata
- PR 4 (planned): group/department scope resolution via MS Graph group membership
- Future: exception workflow endpoint for formal exception_granted state transitions with approval chain
