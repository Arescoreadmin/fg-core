...
### 2026-05-06 — PR 4 Report Generation Hardening

**Branch:** `pr/4-report-hardening`

**Area:** api/reports_engine.py / api/report_jobs.py / tests

---

**Purpose:**

Move report generation lifecycle onto an explicit, auditable job path. The
endpoint already returned 202 + job_id via BackgroundTasks, but lacked: a
formal job state enum, a configurable timeout on the LLM call, stable reason
codes for failures, audit/event logging for each state transition, and tenant
isolation on the GET /reports/{id} read path.

**Root cause / trigger:**

`_generate_report_sync` called `call_provider` (Anthropic LLM) with no
timeout. A hung or slow provider call would hold the BackgroundTask thread
indefinitely. Failures wrote `report.status = "failed"` but emitted no
structured audit record and used no stable reason code. GET /reports/{id} did
not validate that the caller's tenant_id matched `report.tenant_id`.

**Decision — BackgroundTasks (already in repo):**

FastAPI BackgroundTasks are already the existing pattern. No new infra
(Celery, Redis, Temporal) was introduced. The timeout is applied via
`asyncio.wait_for` + `loop.run_in_executor`, keeping the synchronous DB code
unchanged. `_generate_report_sync` bridges to async via `asyncio.run()`.

**Files changed:**

- `api/report_jobs.py` (new) — `ReportJobState` enum (queued/running/succeeded/
  failed) and stable reason codes `REPORT_GENERATION_TIMEOUT`,
  `REPORT_GENERATION_FAILED`.
- `api/reports_engine.py` — added `_emit_report_event()` (audit helper),
  `_generate_report_async()` (asyncio.wait_for wrapper),
  `_handle_timeout()` (timeout recovery), `_do_generate_report()` (renamed
  from `_generate_report_sync`, adds lifecycle audit events). GET /reports/{id}
  enforces tenant isolation (wrong tenant → 404). POST /reports/generate emits
  `report_job_queued` audit event. Configurable via
  `FG_REPORT_GENERATION_TIMEOUT_S` (default 300 s).
- `tests/test_report_jobs.py` (new) — 26 tests covering all lifecycle events,
  reason codes, tenant isolation, and sensitive payload exclusion from audit
  metadata.
- `docs/ai/PR_FIX_LOG.md` — this entry.

**Behavior proof (async, non-blocking):**

Endpoint returns 202 immediately. `background_tasks.add_task(_generate_report_sync, ...)`
schedules work. `_generate_report_sync` calls `asyncio.run(_generate_report_async(...))`
which wraps `_do_generate_report` in `asyncio.wait_for` + executor.

**Timeout/failure proof (reason codes):**

- `asyncio.TimeoutError` → `_handle_timeout` sets `error_message = REPORT_GENERATION_TIMEOUT`
- Any other exception → `report.status = "failed"`, reason_code = `REPORT_GENERATION_FAILED`
- Both codes are constants from `api/report_jobs.py`; tests assert against the constants.

**Audit/event proof:**

`_emit_report_event()` calls `get_auditor().log_event(AuditEvent(...))` for:
`report_job_queued`, `report_job_started`, `report_job_succeeded`, `report_job_failed`.
Each event carries `tenant_id`, `report_id`, `assessment_id`, `job_state`, and on
failure `reason_code`; on completion `duration_ms`.

**Tenant isolation proof:**

GET /reports/{id} checks `request.state.tenant_id` against `report.tenant_id`;
mismatch returns 404 (not 403) to prevent enumeration. Test
`test_report_job_status_enforces_tenant_isolation` asserts 404 for wrong-tenant caller.

**Sensitive payload safety proof:**

`_emit_report_event()` only places `{report_id, assessment_id, job_state, reason_code?,
duration_ms?}` in `details`. No content, prompts, model outputs, or PHI are ever
included. Test `test_report_audit_metadata_does_not_include_sensitive_payload` asserts
forbidden keys are absent.

**Validation results:**

- `pytest -q tests/test_report_jobs.py` → 26 passed ✓
- `pytest -q tests -k "report or worker or queue"` → 44 passed ✓
