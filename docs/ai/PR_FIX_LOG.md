...
**Branch:** `feat/topology-workspace-ui-pr20` (delivered on `claude/relaxed-ramanujan-jtRaH`)

**PR/context:** PR 20 / GitHub PR #387 — Governance Topology Workspace: interactive graph UI rendering the server-computed governance topology graph.

**Area:** Frontend / Console UI / Backend graph API (additive routes only)

**Behavior added:**
- `GET /governance/graph/edges` — list/filter edges by type, source, target; `governance:read` gated.
- `GET /governance/graph/path?from=&to=` — expose existing `find_path()` service (BFS, depth ≤ 10); `governance:read` gated. Answers: "is there a provenance path between two governance entities?"
- `POST /governance/graph/anomalies/{id}/resolve` — mark anomaly inactive (`is_active=False`, `resolved_at=now`); `governance:write` gated. 404 on tenant mismatch, 409 if already resolved.
- BFF proxy rules added for `governance/graph` (GET, POST, HEAD) and `governance/assets` (GET, HEAD).
- `apps/console/lib/governanceApi.ts` — typed API client (12 functions, all query param names matched to backend aliases: `from`, `to`, `/lineage/{id}`, `/rebuild`).
- `apps/console/app/governance/topology/page.tsx` — three-panel workspace: left (AnomalyQueryPanel, PathFinder, CoveragePanel), center (TopologyCanvas), right (NodeInspector, EdgeInspector, ProvenanceViewer, ImpactAnalysis).
- 11 new components under `apps/console/components/governance/topology/`.

**High-risk file changed:** `api/governance_graph.py`

**Reason backend change required:** The UI needs edges listable independently (not just via traversal), a dedicated path endpoint (service function existed but had no route), and anomaly resolution (ORM field existed, no write path). All three are direct reflections of existing service layer capabilities.

**Security impact:**
- No new auth scopes defined. Existing `governance:read` / `governance:write` enforcement reused.
- Tenant isolation: all 3 routes resolve tenant from auth context; `tenant_id` never accepted from request body or query params.
- Anomaly resolve: 404 guard if anomaly.tenant_id ≠ caller; no cross-tenant write path.
- UI: no `dangerouslySetInnerHTML`, no client-side graph computation, no UPNs/credentials rendered, `tenant_id` not surfaced in panels.
- Cytoscape.js (`3.33.4`): browser-only rendering library, loaded via `import()` inside `useEffect`. No network calls, no SSR execution.

**Tenant isolation impact:** No aggregate mixes tenants. All graph queries filter `WHERE tenant_id = ?`. BFF injects `X-Tenant-ID` server-side from `CORE_TENANT_ID` env; never from browser state.

**Governance impact:** Governance topology is now operator-visible and actionable. Anomalies are triageable inline. Governance chain existence between any two nodes is now verifiable. NIST-AI-RMF coverage is visible in the workspace.

**Files changed:**
- `api/governance_graph.py` — 3 new routes (edges, path, anomaly resolve)
- `apps/console/app/api/core/[...path]/route.ts` — 2 new proxy rule entries
- `apps/console/lib/governanceApi.ts` — new (12 typed API functions)
- `apps/console/app/governance/topology/page.tsx` — new
- `apps/console/components/governance/topology/` — 11 new components
- `apps/console/package.json` — `cytoscape@^3.33.4` + `@types/cytoscape` added
- `apps/console/package-lock.json` — updated
...
- `docs/SOC_ARCH_REVIEW_2026-02-15.md` — PR 20 entry
- `docs/SOC_EXECUTION_GATES_2026-02-15.md` — PR 20 entry
- `docs/ai/PR_FIX_LOG.md` — this entry

**CI repair (post-initial-commit fixes applied in this PR):**
- Removed invalid `// eslint-disable-next-line @typescript-eslint/no-explicit-any` comment in `TopologyCanvas.tsx` — `@typescript-eslint` plugin not in this project's ESLint config; the comment itself caused the build failure.
- Fixed `style: CY_STYLE as Parameters<typeof cytoscape>[0]['style']` TypeScript error — `Parameters<typeof cytoscape>[0]` resolves to `String` not `CytoscapeOptions`; changed to `as any`.
- Fixed 4 API client query param mismatches: `from_node→from`, `to_node→to` (backend uses `Query(alias=...)`), `/nodes/{id}/lineage→/lineage/{id}` (wrong route shape), `/build→/rebuild` (wrong route name).
- Ran `ruff format api/governance_graph.py` (formatting-only diff).
- Ran `make route-inventory-generate` (3 new routes not in inventory).
- Ran `make contract-authority-refresh` (OpenAPI spec changed by routes-gen; marker outdated).

**Validation results:**
- `npm run build` → ✓ compiled, 0 errors, 0 warnings; `/governance/topology` in build output
- `npm test` → 1019 passed, 0 failed
- `ruff check api/governance_graph.py` → all checks passed
- `python tools/ci/guard_pr_fix_log.py --base origin/main --head HEAD` → OK
- `python scripts/contract_authority_check.py` → ✓ Contract authority markers match
- `make fg-fast` → pending full run after this commit
