...
_WIDGET_MAX_PAYLOAD_BYTES = int(
    os.getenv("FG_DASH_WIDGET_MAX_PAYLOAD_BYTES", str(64 * 1024))
)
...
def _provider_system_health(
    *, tenant_id: str, context: DashboardContext
) -> dict[str, Any]:
    _ = context
    return {"tenant_id": tenant_id, "status": "ok"}


def _provider_recent_decisions(
    *, tenant_id: str, context: DashboardContext
) -> dict[str, Any]:
...
def _run_widget(
    widget_id: str, tenant_id: str, context: DashboardContext
) -> ProviderResult:
...
def _etag_key(
    *,
    ...
    versions = {
        wid: (get_widget_contract(wid) or {}).get("version", "na") for wid in widgets
    }
    filter_hash = hashlib.sha256(
        json.dumps(context.model_dump(), sort_keys=True, separators=(",", ":")).encode(
            "utf-8"
        )
    ).hexdigest()
    blob = json.dumps(
...
def snapshot(
    ...
    filter_hash = hashlib.sha256(
        json.dumps(
            payload.context.model_dump(), sort_keys=True, separators=(",", ":")
        ).encode("utf-8")
    ).hexdigest()
    for wid in payload.widget_ids:
        allowed, deny_reason = widget_allowed(
            tenant_id=target_tenant, persona=persona, widget_id=wid
        )
