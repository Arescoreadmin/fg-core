...
def _ensure_models_imported() -> None:
    ...
    importlib.import_module(
        "api.db_models_governance_adaptive_intelligence"
...
def _auto_migrate_sqlite(engine: Engine) -> None:
    ...
    with engine.begin() as conn:
        ...
        # PR 14.6.2 P1: Timeline Authority — add new columns if missing
        for col, decl in (
            ("authority_level", "TEXT NOT NULL DEFAULT 'SYSTEM'"),
            ("signature_algorithm", "TEXT NOT NULL DEFAULT ''"),
            ("signature_value", "TEXT NOT NULL DEFAULT ''"),
            ("signed_at", "TEXT"),
            ("external_reference", "TEXT NOT NULL DEFAULT ''"),
            ("external_reference_type", "TEXT NOT NULL DEFAULT ''"),
...
def init_db(*, sqlite_path: Optional[str] = None) -> None:
    ...
    try:
        ...
        if "tenant_credentials" not in tables and engine.dialect.name == "sqlite":
            logger.warning(
                "Expected table 'tenant_credentials' missing; tables=%s", sorted(tables)
            )
    except Exception:
        pass
