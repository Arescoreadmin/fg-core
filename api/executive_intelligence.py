...
Security:
  tenant isolation, governance:read scope, audit logging, no cross-tenant leakage
"""
...
def _base_envelope(tenant_id: str, inputs: list[Any], confidence: float, evidence_count: int, calculation: str) -> dict[str, Any]:
    now = _now()
    return {
        "tenant_id": tenant_id,
...
def executive_overview(request: Request) -> dict[str, Any]:
    ...
    return {
        **_base_envelope(tenant_id, inputs, 0.95, evidence_count, "Aggregate counts from decisions, compliance_findings, security_audit_log, compliance_requirements"),
...
def executive_posture(request: Request) -> dict[str, Any]:
    ...
    return {
        **_base_envelope(tenant_id, inputs, 0.88, evidence_count, "Posture = 100 - (risk_score/10) - (failure_rate*50); clamped 0-100"),
...
def executive_risk(request: Request) -> dict[str, Any]:
    ...
    top_risks = sorted(open_findings, key=lambda f: _SEVERITY_WEIGHT.get((f.severity or "info").lower(), 0), reverse=True)[:10]
    ...
    return {
        **_base_envelope(tenant_id, inputs, 0.92, len(findings), "Risk heatmap from compliance_findings severity aggregation"),
        "heatmap": heatmap,
        "top_risks": [
            {
...
def executive_compliance(request: Request) -> dict[str, Any]:
    ...
    with Session(get_engine()) as db:
        requirements = _safe(
            lambda: db.query(ComplianceRequirementRecord)
            .filter(ComplianceRequirementRecord.tenant_id == tenant_id)
            .all(),
            [],
        )
    ...
    open_findings = _open_findings(findings)
    findings_by_req: dict[str, list[ComplianceFindingRecord]] = {}
    for f in open_findings:
        for req_id in (f.req_ids_json or []):
            findings_by_req.setdefault(str(req_id), []).append(f)
    ...
    for fw in _FRAMEWORKS:
        matched_reqs = [r for r in requirements if fw.lower() in (r.source or "").lower() or fw.lower() in (r.source_name or "").lower()]
        matched_findings = [f for f in open_findings if any(fw.lower() in str(req_id).lower() for req_id in (f.req_ids_json or []))]
        frameworks[fw] = {
    ...
    return {
        **_base_envelope(tenant_id, inputs, 0.9, req_total + len(findings), "Compliance coverage from compliance_requirements grouped by source; findings cross-referenced by req_ids_json"),
...
def executive_business(request: Request) -> dict[str, Any]:
    ...
    cost_breakdown = {
        sev: {"count": cnt, "unit_cost": _COST_PER_FINDING.get(sev, 0), "total": cnt * _COST_PER_FINDING.get(sev, 0)}
        for sev, cnt in sev.items()
    }
    ...
    return {
        **_base_envelope(tenant_id, inputs, 0.7, len(open_findings), "Cost of risk = SUM(count_per_severity * assumed_cost_per_finding). Assumptions: critical=$50k, high=$15k, medium=$4k, low=$500, info=$50. Insurance readiness = 100 - (critical*20) - (high*8)"),
...
def executive_trends(request: Request) -> dict[str, Any]:
    ...
    return {
        **_base_envelope(tenant_id, inputs, 0.95, len(findings) + len(decisions_rows), "Trend counts grouped by created_at into 30/90/180/365 day windows relative to query time"),
        "windows": trend_data,
        "snapshot_ts": now_ts,
        "data_available": True,
    }
...
def executive_recommendations(request: Request) -> dict[str, Any]:
    ...
    return {
        **_base_envelope(tenant_id, inputs, 1.0, len(open_findings), "Top 25 open findings sorted by severity weight DESC. No AI generation — deterministic sort."),
...
def executive_forecast(request: Request) -> dict[str, Any]:
    ...
    for days in windows:
        ...
        period = [
            f for f in findings
            if f.created_at and prev_cutoff <= f.created_at < cutoff
        ]
    ...
    return {
        **_base_envelope(tenant_id, inputs, round(confidence, 3), len(findings), "Linear regression over 6 × 30-day trailing windows. Formula: y=mx+b, slope from OLS. Confidence = R² * 0.85, clamped [0.3, 0.85]."),
...
def executive_priorities(request: Request) -> dict[str, Any]:
    ...
    def _priority_score(f: ComplianceFindingRecord) -> float:
    ...
    sorted_findings = sorted(open_findings, key=_priority_score, reverse=True)
    ...
    return {
        **_base_envelope(tenant_id, inputs, 1.0, len(open_findings), "Top 15 open findings ranked by priority_score = severity_weight + recency_bonus"),
...
def executive_summary(request: Request) -> dict[str, Any]:
    ...
    top_finding = max(open_findings, key=lambda f: _SEVERITY_WEIGHT.get((f.severity or "info").lower(), 0), default=None)
    ...
    return {
        **_base_envelope(tenant_id, inputs, 0.88, evidence_count, "Board-ready summary compiled from compliance_findings, decisions, security_audit_log, compliance_requirements"),
        "executive_headline": {
