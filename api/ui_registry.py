...
@lru_cache(maxsize=1)
def _views_validator() -> Draft202012Validator:
    return Draft202012Validator(load_json_contract(_VIEWS_SCHEMA_PATH, root=_DASHBOARD_ROOT))


@lru_cache(maxsize=1)
def _views_contract() -> dict[str, Any]:
    payload = load_json_contract(_VIEWS_PATH, root=_DASHBOARD_ROOT)
    errs = sorted(_views_validator().iter_errors(payload), key=lambda e: list(e.path))
    if errs:
...
def _filtered_dashboard(dashboard: dict[str, Any], scopes: set[str], *, tenant_id: str, persona: str) -> dict[str, Any]:
    ...
    for widget_id in dashboard.get("widgets", []):
        ...
        if required and not required.issubset(scopes):
            continue
        allowed, reason = widget_allowed(tenant_id=tenant_id, persona=persona, widget_id=widget_id)
        if not allowed:
            continue
        kept_widgets.append({"id": widget_id, "contract": contract, "policy_reason": reason})
    return {
        "id": dashboard.get("id"),
...
def get_dashboards(request: Request) -> dict[str, Any]:
    ...
    for d in views.get("dashboards", []):
        ...
        if allowed and persona not in allowed:
            continue
        dashboards.append(_filtered_dashboard(d, scopes, tenant_id=tenant_id, persona=persona))
