...
def _theme_validator() -> Draft202012Validator:
    schema = load_json_contract(_THEME_SCHEMA_PATH, root=_THEME_ROOT)
...
def _override_params_hash(*, source_tenant: str, target_tenant: str, reason: str) -> str:
    import json
...
def sanitize_css(css: str | None) -> str | None:
    ...
    banned_patterns = [r"@import", r"url\s*\(", r"expression\s*\(", r"</style>", r"<", r">"]
...
def _theme_for_tenant(tenant_id: str) -> dict[str, Any]:
    ...
    errs = list(_theme_validator().iter_errors(payload))
    if errs:
