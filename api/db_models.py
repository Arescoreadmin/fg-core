...
class AIDeviceRegistry(Base):
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "device_id", name="uq_ai_device_registry_tenant_device"),
    ...
    last_seen_at = Column(DateTime(timezone=True), nullable=False, default=utcnow)
    telemetry_json = Column(JSON, nullable=False, default=dict, server_default=text("'{}'"))
...
class AITokenUsage(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_ai_token_usage_tenant_device_day", "tenant_id", "device_id", "usage_day"),
    )
...
class AIQuotaDaily(Base):
    ...
    __table_args__ = (
        UniqueConstraint("quota_scope", "usage_day", name="uq_ai_quota_daily_scope_day"),
