...
class ConnectorPolicySetRequest(_StrictModel):
    version: str = Field(min_length=1)


class ConnectorIngestPayload(_StrictModel):
...
class ConnectorIngestRequest(_StrictModel):
    ...
    payload: ConnectorIngestPayload = Field(default_factory=ConnectorIngestPayload)


class ConnectorCredentialConnectRequest(_StrictModel):
...
class ConnectorStateSetRequest(_StrictModel):
    enabled: bool
...
def set_connectors_policy(
    request: Request,
    body: ConnectorPolicySetRequest,
    idempotency_key: str | None = Header(default=None, alias="Idempotency-Key"),
    db: Session = Depends(tenant_db_required),
) -> dict[str, Any]:
    tenant_id = require_bound_tenant(request)
...
def connect_connector_credentials(
    connector_id: str,
    request: Request,
    body: ConnectorCredentialConnectRequest,
    idempotency_key: str | None = Header(default=None, alias="Idempotency-Key"),
    db: Session = Depends(tenant_db_required),
) -> dict[str, Any]:
    tenant_id = require_bound_tenant(request)
