...
def _prod_like() -> bool:
    return (os.getenv("FG_ENV") or "").strip().lower() in {"prod", "production", "staging"}
...
def _provider_env_allowed(provider: str) -> bool:
    flags = {
        "simulated": (os.getenv("FG_AI_ENABLE_SIMULATED") or "1" if not _prod_like() else "0").strip().lower()
        in {"1", "true", "yes", "on"}
    }
...
def _validate_device_signature_stub(request: Request, tenant_id: str, device_id: str, request_hash: str) -> None:
    ...
    try:
        ...
        raise _error(401, "AI_DEVICE_SIG_INVALID", "invalid signature timestamp") from exc
    ...
    if not secret:
        if _prod_like():
            raise _error(503, "AI_DEVICE_SIG_CONFIG_INVALID", "signature secret missing")
        return
    ...
    *,
    ...
    try:
        if tenant_limit > 0:
            ...
            if t_row is None:
                raise _error(429, "AI_QUOTA_TENANT_EXCEEDED", "tenant token quota exceeded")
        ...
        if device_limit > 0:
            ...
            if d_row is None:
                raise _error(429, "AI_QUOTA_DEVICE_EXCEEDED", "device token quota exceeded")
...
def _contains_pii(value: str, deny_terms: list[str]) -> bool:
    ...
    if "ssn" in low or "social security" in low:
        return True
    if any(ch.isdigit() for ch in value) and len([ch for ch in value if ch.isdigit()]) >= 13:
        return True
    return False
...
def ai_usage(request: Request, db: Session = Depends(tenant_db_required)) -> dict[str, Any]:
    tenant_id = require_bound_tenant(request)
    device_id = _device_id(request, None)
    if not device_id:
...
def ai_chat(
    ...
    if provider not in set(policy.get("allowed_providers") or []):
        raise _error(400, "AI_PROVIDER_DENIED_BY_TENANT_POLICY", "provider not allowed by tenant policy")
    if provider not in _global_allowed_providers():
        raise _error(400, "AI_PROVIDER_DENIED_BY_SERVER", "provider not allowed by server config")
    if not _provider_env_allowed(provider):
        raise _error(400, "AI_PROVIDER_DENIED_BY_ENV", "provider not allowed in this environment")
    ...
    session_id = hashlib.sha256(f"{tenant_id}|{device_id}|{request_day}".encode("utf-8")).hexdigest()[:24]
    event_id = hashlib.sha256(f"{tenant_id}|{device_id}|{request_hash}|{policy_hash}".encode("utf-8")).hexdigest()[:24]
    ...
    usage_record_id = hashlib.sha256(f"{tenant_id}|{device_id}|{request_hash}|{session_id}".encode("utf-8")).hexdigest()
    ...
    try:
        if _contains_pii(payload.message, deny_terms):
            blocked_error = _error(400, "AI_INPUT_POLICY_BLOCKED", "input blocked by policy")
            raise blocked_error
        ...
        if _contains_pii(output, deny_terms):
            blocked_error = _error(400, "AI_OUTPUT_POLICY_BLOCKED", "output blocked by policy")
            raise blocked_error
        ...
        if total_tokens > max_tokens:
            blocked_error = _error(400, "AI_REQUEST_TOKEN_CAP_EXCEEDED", "request token cap exceeded")
            raise blocked_error
    ...
    if metering_mode == "unknown":
        denied = _error(503, "AI_METERING_UNCERTAIN", "unable to determine metering mode")
        _audit(
            EventType.ADMIN_ACTION,
            tenant_id=tenant_id,
            success=False,
...
def _toggle_device(
    ...
    db.execute(
        ...
        {"enabled": 1 if enabled else 0, "tenant_id": tenant_id, "device_id": device_id},
    )
    db.commit()
    ...
    get_auditor().log_admin_action(
        ...
        request=request,
        details={
            "actor": getattr(getattr(request.state, "auth", None), "key_prefix", "unknown"),
            "device_id": device_id,
            "reason": reason,
            "ticket": ticket,
...
def admin_enable_device(
    ...
    tenant_id = require_bound_tenant(request)
    return _toggle_device(db, request, tenant_id, device_id, True, payload.reason, payload.ticket)


@admin_router.post(
    "/devices/{device_id}/disable", dependencies=[Depends(require_scopes("admin:write"))]
)
