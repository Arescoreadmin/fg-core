...
# --- FIXED: real gate target, not a Makefile crime scene ---
check-decision-roe: venv
	@PYTHONPATH=. $(PY) tools/ci/check_decision_no_duplicate_roe_fields.py
...
.PHONY: pr-soc-prep
pr-soc-prep:
	@echo "==> Updating SOC artifacts (sync)"
...
FG_FAST_MAX_SECONDS ?= 480
FG_FAST_WARN_SECONDS ?= 420
...
pip-audit: venv
	...
	@$(PY) -m pip_audit --ignore-vuln CVE-2026-4539 --ignore-vuln PYSEC-2025-183 -r requirements.txt -r requirements-dev.txt
	@$(PY) -m pip_audit --ignore-vuln CVE-2026-4539 --ignore-vuln PYSEC-2025-183 -r admin_gateway/requirements.txt -r admin_gateway/requirements-dev.txt
...
.PHONY: pg-reset-frostgate test-pg-migrations-replay route-inventory-update
