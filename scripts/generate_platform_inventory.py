...
def main() -> int:
    ...
    for p in PLANE_REGISTRY:
        ...
        routes = sorted({f"{r['method']} {r['path']}" for r in route_inventory if any(str(r["path"]).startswith(prefix) for prefix in p.route_prefixes)})
        by_plane_routes[pid] = routes
        missing_targets = sorted([t for t in p.required_make_targets if t not in make_targets])
        if missing_targets:
        ...
        for e in p.evidence:
            ...
            if not sp.exists() or not gp.exists():
                missing_evidence.append({"schema": e.schema_path, "generator": e.generator_script})
        if missing_evidence:
            gaps.append({"type": "missing_evidence_components", "plane": pid, "details": missing_evidence, "suggested_fix": "Add missing schema/generator files declared in plane registry."})
        ...
        planes.append({
            ...
            "evidence": sorted([{"schema": e.schema_path, "generator": e.generator_script} for e in p.evidence], key=lambda x: (x["schema"], x["generator"]))
        })

    unexpected = sorted({str(r["path"]) for r in route_inventory if str(r["path"]).startswith("/") and not any(str(r["path"]).startswith(prefix) for prefix in owned_prefixes)})
    if unexpected:
    ...
    readiness = {
        "tenant_binding_coverage": all(bool(r.get("tenant_bound")) or str(r["path"]).startswith("/health") for r in route_inventory),
    ...
    (art / "platform_inventory.json").write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")

    inv_md = ["# Platform Inventory", "", "## Planes"]
    for p in payload["planes"]:
        inv_md.append(f"- `{p['plane_id']}` flags=`{p['mount_flag']}` targets={', '.join(p['required_make_targets'])}")
    ...
    if payload["gaps"]:
        for g in payload["gaps"]:
            gap_md.append(f"- [{g['type']}] {g.get('plane', 'global')}: {g['details']} | fix: {g['suggested_fix']}")
    else:
        gap_md.append("- none")
