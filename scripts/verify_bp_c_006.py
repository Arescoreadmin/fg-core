...
GATE_ID = "BP-C-006"
INVARIANT = "Governance UI must be read-only by default and governance writes must be auditable."
EXPECTED_ALIGN = "make bp-c-006-gate"
REPORT_PATH = Path("artifacts/gates/bp_c_006_report.json")
...
def main() -> int:
    ...
    checks = [
        (
            Path("contracts/core/openapi.json"),
            "/governance/changes",
            "missing governance listing endpoint",
        ),
        (
            Path("contracts/core/openapi.json"),
            "/governance/changes/{change_id}/approve",
            "missing governance audited approve endpoint",
        ),
    ]
