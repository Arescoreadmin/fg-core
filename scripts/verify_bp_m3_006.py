...
GATE_ID = "BP-M3-006"
INVARIANT = "Delivery failures (DLQ-equivalent) must be surfaced as compliance failures, not silent drops."
EXPECTED_ALIGN = "make bp-m3-006-gate"
REPORT_PATH = Path("artifacts/gates/bp_m3_006_report.json")
...
def main() -> int:
    ...
    required = [
        ...
        Path("tests/test_tripwire_delivery.py"),
    ]
    ...
    checks = [
        (
            Path("tests/test_tripwire_delivery.py"),
            "webhook_failed",
            "missing failed-delivery compliance assertion",
        ),
        (
            Path("tests/test_tripwire_delivery.py"),
            "webhook_retry",
            "missing retry/dlq handling assertion",
        ),
    ]
