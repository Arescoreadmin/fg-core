...
| PR 29 | HIPAA Playbook | 🔄 in progress | Dedicated HIPAA governance execution playbook; Privacy Officer + Security Officer gates; 7 HIPAA-specific document classes; annual evidence freshness |
...
| 8 | **HIPAA playbook** — `services/field_assessment/playbooks.py` HIPAA falls back to `comprehensive`; banking clients need dedicated gates | — | PR 29 | ✅ done — `HIPAA_PLAYBOOK` with Privacy Officer + Security Officer required interview gates; 7 HIPAA document classes (BAA, PHI inventory, risk analysis, sanction policy, access control policy, incident response, training records); annual freshness on risk analysis, PHI inventory, sanction policy, training records; BAA has no expiry |
