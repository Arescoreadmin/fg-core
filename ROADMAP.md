...
| **Phase 1 — Trusted Pilot** | ✅ H12 durable scan jobs (PR fix 50): DurableJobService, all 9 scan routes create FaScanJob, lease model (hostname:pid, 300s TTL), exponential backoff retry, dead-letter routing, orphan recovery, DB fallback in status route, `GET /scan-jobs` + `GET /scan-jobs/{job_id}` routes, migration 0084, 21 tests; H11 drift RLS, H14 console RBAC + actor attribution, PI20 FA/Gov outbox boundary, DB hardening, portal session identity | Weeks 3–6 |
