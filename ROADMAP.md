...
*Last updated: 2026-06-03 (H15 implemented — EvidenceLifecycleService: 3-state machine (collected→locked→legal_hold), direct collected→legal_hold immediate preservation path (addendum requirement), bulk QA-approval lock in qa_approve_report_route, route guards on update_observation_route + delete_observation_route + patch_questionnaire_response, DB triggers on fa_field_observations/fa_scan_results/fa_document_analyses/fa_evidence_links (UPDATE legal_hold + downgrade, DELETE locked/legal_hold) and governance_reports (report content immutability when finalized), chain-of-custody FaEvidenceLifecycleEvent + FaLegalHold models, migration 0083, 31 security tests. H13.5 — AuditCoverageValidator: AST-based gate auto-discovers 38 mutation routes, validates against audit_exceptions.yaml, `make audit-coverage-check` mandatory in fg-fast, 25 security tests, 100% coverage. H13 fixed — AuditAtomicityService: report split-commit fixed, 5 unaudited paths fixed, transaction_id/entity correlation columns, migration 0082, 33 tests.)*
