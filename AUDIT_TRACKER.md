...
*Last updated: 2026-06-02 — Codex forensic audit added C5/C6/C7 (audio proxy SSRF, scanner SSRF, portal credential model) and H11–H15 (drift RLS, non-durable jobs, audit atomicity, console RBAC, evidence immutability) and PI17–PI20 (drift-report mutation, UI/API contract drift, scheduler registry-only, FA/Gov coupling). All items tracked in ENTERPRISE_PLAN.md with phased delivery order. Previously: 15-module SQLite FA forensic regression suite; PI16 fixed (terminal evidence mutation locks); H7, H8, PI14 fixed; C4 fixed (PortalClientScopeMiddleware); C1 fixed (QA gate enforcement); C2 fixed (create_all before apply_migrations); C3 fixed (migration 0075 — RLS on all 11 FA/governance tables); H5 fixed (migration 0076 — append-only triggers); H9 fixed (audio-evidence validator + audit diff); H10 fixed (QA approval immutability); PI15 fixed (migration 0077); PI13 fixed (exhaustive pagination); PI11 fixed (msgraph_bridge lock); PI12 fixed (corpus feed three loops); H6 partially closed (private blob + auth proxy; OpenAI governance deferred).*
