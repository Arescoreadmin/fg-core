...
def _payload() -> dict[str, object]:
    return json.loads(Path("tools/ci/artifact_policy_allowlist.json").read_text(encoding="utf-8"))
...
def test_artifact_policy_allowlist_has_expected_keys():
    ...
    assert any(x.endswith("SOC_AUDIT_GATES.md") for x in payload["allowed_committed_artifacts"])
