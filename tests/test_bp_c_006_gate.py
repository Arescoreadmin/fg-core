...
SCRIPT = Path(__file__).resolve().parents[1] / "scripts" / "verify_bp_c_006.py"
...
def _repo(tmp_path: Path) -> Path:
    ...
    _write(
        ...
        json.dumps({"BP-C-006": "make bp-c-006-gate"}) + "\n",
    )
...
def test_negative_proof(tmp_path: Path) -> None:
    ...
    report = json.loads(
        (repo / "artifacts/gates/bp_c_006_report.json").read_text(encoding="utf-8")
    )
    assert any("missing governance listing endpoint" in e for e in report["errors"])
