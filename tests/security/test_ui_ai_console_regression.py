...
def test_device_enable_disable_gate(build_app):
    ...
    assert deny_enable.json()["error_code"]
...
def test_unknown_device_denied(build_app):
    ...
    assert resp.json()["error_code"] == "unknown_device"
...
def test_provider_denied_by_server_config(build_app, monkeypatch):
    ...
    assert resp.json()["error_code"] == "provider_not_allowed"
...
def test_request_token_cap_exceeded(build_app, monkeypatch):
    ...
    assert resp.status_code == 400
    assert resp.json()["error_code"] == "request_token_cap_exceeded"


def test_admin_devices_includes_items_compat_shape(build_app):
    client = _client(build_app)
    hdrs = _headers("ui:read", "ai:chat", "admin:write", tenant_id="tenant-dev")

    resp = client.get("/admin/devices", headers=hdrs)
    assert resp.status_code == 200
    body = resp.json()
...
def test_chat_usage_includes_prompt_tokens(build_app):
    client = _client(build_app)
    hdrs = _headers("ui:read", "ai:chat", "admin:write", tenant_id="tenant-dev")
    exp = client.get("/ui/ai/experience", headers=hdrs).json()
    device_id = exp["device"]["device_id"]
    ...
    resp = client.post(
        "/ui/ai/chat",
        headers=hdrs,
        json={"message": "hello tokens", "device_id": device_id},
    )
    assert resp.status_code == 200
...
def test_signature_hook_requires_headers_when_enabled(build_app, monkeypatch):
    monkeypatch.setenv("FG_AI_SIGNATURE_HOOK_ENABLED", "1")
    client = _client(build_app)
    hdrs = _headers("ui:read", "ai:chat", tenant_id="tenant-dev")
    resp = client.get("/ui/ai/experience", headers=hdrs)
    assert resp.status_code == 401


def test_day_bucket_helper_exists_and_uses_utc_day():
    from datetime import UTC, datetime

    from api import ui_ai_console

    day = ui_ai_console._day_bucket(datetime(2026, 2, 18, 23, 59, tzinfo=UTC))
    assert day == "2026-02-18"
