...
def test_redacted_token_in_doc_does_not_fail(tmp_path: Path) -> None:
    ...
    violations = _violations(
        tmp_path, "# The credential was [REDACTED_EXPOSED_PASSWORD]\n"
    )
    assert violations == []
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "FG_DB_URL",
            "postgresql+psycopg://fg_user:realpassword@postgres:5432/frostgate",
        ),
...
def test_is_secret_var(key: str, expected: bool) -> None:
    assert _is_secret_var(key) is expected, (
        f"_is_secret_var({key!r}) expected {expected}"
    )
