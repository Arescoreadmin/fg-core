...
def test_widget_timeout_degrades(build_app, monkeypatch):
    ...
    monkeypatch.setitem(__import__("api.ui_dashboard_data", fromlist=["_PROVIDERS"])._PROVIDERS, "recent_decisions", slow_provider)

    resp = client.post(
        "/ui/dashboard-data/snapshot",
        headers={"X-API-Key": key},
        json={"dashboard_id": "posture", "widget_ids": ["recent_decisions"], "context": {"q": "timeout-a"}},
    )
...
def test_widget_payload_cap(build_app, monkeypatch):
    mod = __import__("api.ui_dashboard_data", fromlist=["_WIDGET_MAX_PAYLOAD_BYTES", "_CACHE"])
    ...
    monkeypatch.setitem(__import__("api.ui_dashboard_data", fromlist=["_PROVIDERS"])._PROVIDERS, "recent_decisions", huge_provider)

    resp = client.post(
        "/ui/dashboard-data/snapshot",
        headers={"X-API-Key": key},
        json={"dashboard_id": "posture", "widget_ids": ["recent_decisions"], "context": {"q": "payload-a"}},
    )
    assert resp.status_code == 200
...
def test_degrade_redacts_error(build_app, monkeypatch):
    ...
    monkeypatch.setitem(__import__("api.ui_dashboard_data", fromlist=["_PROVIDERS"])._PROVIDERS, "recent_decisions", bad_provider)

    resp = client.post(
        "/ui/dashboard-data/snapshot",
        headers={"X-API-Key": key},
        json={"dashboard_id": "posture", "widget_ids": ["recent_decisions"], "context": {"q": "err-a"}},
    )
    assert resp.status_code == 200
...
def test_cache_isolation_different_tenant_etag(build_app):
    ...
    body = {"dashboard_id": "posture", "widget_ids": ["system_health"], "context": {"q": "cache-1"}}
    ra = client.post("/ui/dashboard-data/snapshot", headers={"X-API-Key": key_a}, json=body)
    rb = client.post("/ui/dashboard-data/snapshot", headers={"X-API-Key": key_b}, json=body)
...
def test_cache_isolation_override_flag(build_app, monkeypatch):
    ...
    body = {"dashboard_id": "posture", "widget_ids": ["system_health"], "context": {"q": "cache-ovr"}}
    base = client.post("/ui/dashboard-data/snapshot", headers={"X-API-Key": key}, json=body)
...
def test_runtime_policy_precedence_reason_codes(build_app):
    ...
    try:
            ...
            json.dumps(
                {
                    ...
                    "persona_overrides": {"analyst": {"disabled_widgets": ["recent_decisions"]}},
                    "tenant_overrides": {"tenant-a": {"disabled_widgets": ["system_health"]}},
                    "feature_flag_overrides": [
                        {"env_flag": "FG_FLAG_DISABLE_DRIFT", "widget_id": "drift_status", "enabled": False}
                    ],
                    "disabled": []
                }
            ),
        ...
        assert s2.json()["errors"]["recent_decisions"] == "WIDGET_NOT_ALLOWED_FOR_PERSONA"
...
def test_degraded_response_not_cached(build_app, monkeypatch):
    ...
    monkeypatch.setitem(__import__("api.ui_dashboard_data", fromlist=["_PROVIDERS"])._PROVIDERS, "recent_decisions", flaky_provider)

    body = {"dashboard_id": "posture", "widget_ids": ["recent_decisions"], "context": {"q": "nocache-degraded"}}
    first = client.post("/ui/dashboard-data/snapshot", headers={"X-API-Key": key}, json=body)
    assert first.status_code == 200
    assert first.json()["errors"]["recent_decisions"] == "provider_failed"

    second = client.post("/ui/dashboard-data/snapshot", headers={"X-API-Key": key}, json=body)
...
def test_widget_logging_redaction_safe(build_app, monkeypatch):
    ...
    resp = client.post(
        "/ui/dashboard-data/snapshot",
        headers={"X-API-Key": key},
        json={"dashboard_id": "posture", "widget_ids": ["system_health"], "context": {"q": "sensitive-filter"}},
    )
    assert resp.status_code == 200
    assert records
