...
# ---------------------------------------------------------------------------
# 15) test_rotate_credential_mint_failure_keeps_old_credential_active
# ---------------------------------------------------------------------------


def test_rotate_credential_mint_failure_keeps_old_credential_active(
    credential_env, monkeypatch
):
    """If minting the new credential fails during rotation, the old credential
    must remain active so the customer does not lose access.
    """
    from unittest.mock import patch

    record, old_secret = create_credential("tenant-rotate-safe")

    # Simulate mint_key raising after the lookup step
    with patch("api.credentials.mint_key", side_effect=RuntimeError("DB unavailable")):
        with pytest.raises((RuntimeError, HTTPException)):
            rotate_credential(record.credential_id, "tenant-rotate-safe")

    # Old credential must still be valid — customer retains access
    tenant = validate_credential(old_secret)
    assert tenant == "tenant-rotate-safe"


# ---------------------------------------------------------------------------
# 16) test_revoke_or_rotate_handles_legacy_api_keys_schema_without_name_column
# ---------------------------------------------------------------------------


def test_revoke_or_rotate_handles_legacy_api_keys_schema_without_name_column(
    tmp_path, monkeypatch
):
    """revoke_credential and rotate_credential must work on legacy api_keys tables
    that do not have the optional `name` column.

    Older schemas may lack this column; _lookup_row must probe via PRAGMA and
    fall back to the 3-column query rather than raising sqlite3.OperationalError.
    """
    ...
    # Build a minimal legacy schema without the `name` column
    ...
    # Mint a key directly into this legacy schema
    from api.auth_scopes.helpers import _get_key_pepper, _key_lookup_hash
    from api.auth_scopes.mapping import mint_key as _mint_key

    raw_key = _mint_key("credential:use", tenant_id="tenant-legacy", ttl_seconds=3600)
    secret_part = raw_key.rsplit(".", 1)[-1]
    pepper = _get_key_pepper()
    cred_id = _key_lookup_hash(secret_part, pepper)

    # revoke_credential must not raise OperationalError on the name-less schema
    from api.credentials import revoke_credential

    revoke_credential(cred_id, "tenant-legacy")  # must not raise

    # Verify it is actually revoked in the DB
    con2 = _sqlite3.connect(db_path)
    try:
        row = con2.execute(
            "SELECT enabled FROM api_keys WHERE key_lookup = ? LIMIT 1", (cred_id,)
        ).fetchone()
    finally:
        con2.close()
    assert row is not None
    assert row[0] == 0  # enabled=0 confirms revocation
