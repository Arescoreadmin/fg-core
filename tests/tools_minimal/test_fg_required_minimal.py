...
class FgRequiredMinimalTests(unittest.TestCase):
    def test_lane_order_integrity(self) -> None:
        self.assertEqual(
            fg_required.LANES,
            ("policy-validate", "required-tests-gate", "fg-fast", "fg-contract", "fg-security"),
        )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with mock.patch.object(fg_required, "ARTIFACT_ROOT", Path(tmp)):
                with self.assertRaises(SystemExit):
                    fg_required._verify_required_files(results=[], generated_reports=[], strict=True)
    ...
    def test_summary_ui_shape_and_hashes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(fg_required, "ARTIFACT_ROOT", root):
                for name in ("required-tests-gate.json", "contract-drift.json", "security-invariants.json"):
                ...
                results = [
                    fg_required.LaneResult("policy-validate", "passed", 1, False, artifact_paths={"lane_log": "a", "lane_triage": "b"}),
                ]
                fg_required._write_summary(results, "passed", 480, 12, hashes)
                payload = json.loads((root / "fg-required-summary.json").read_text(encoding="utf-8"))
    ...
    def test_budget_exceeded_mid_run_is_lane_aware(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            with mock.patch.object(fg_required, "ARTIFACT_ROOT", root):
                with mock.patch.object(fg_required, "_check_working_tree_clean", return_value=None):
                    (root / "lanes" / "policy-validate").mkdir(parents=True, exist_ok=True)
                    (root / "lanes" / "policy-validate" / "lane.log").write_text("ok\n", encoding="utf-8")
                    (root / "lanes" / "policy-validate" / "lane.triage.json").write_text("{}\n", encoding="utf-8")
                    with mock.patch.object(fg_required, "_run_lane", side_effect=[fg_required.LaneResult("policy-validate", "passed", 1, False, artifact_paths={"lane_log": str(root / "lanes/policy-validate/lane.log"), "lane_triage": str(root / "lanes/policy-validate/lane.triage.json")})]):
                        with mock.patch("time.monotonic", side_effect=[0.0, 1.0, 500.0, 501.0]):
                            argv = ["fg_required.py", "--global-budget-seconds", "480", "--strict", "--dry-run"]
                            with mock.patch("sys.argv", argv):
                                rc = fg_required.main()
                self.assertEqual(rc, 1)
                self.assertTrue((root / "fg-required-summary.json").exists())
                self.assertTrue((root / "fg-required-summary.md").exists())
                self.assertTrue((root / "lanes" / "required-tests-gate" / "lane.log").exists())
                self.assertTrue((root / "lanes" / "required-tests-gate" / "lane.triage.json").exists())
    ...
    def test_blocked_command_is_path_aware(self) -> None:
        self.assertFalse(fg_required._is_blocked_command(("python", "-c", "print(\"docs/.env.example\")")))

    def test_working_tree_mutation_detected_after_lane(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            with mock.patch.object(fg_required, "ARTIFACT_ROOT", root):
                with mock.patch.object(fg_required, "_run_lane", return_value=fg_required.LaneResult("policy-validate", "passed", 1, False, artifact_paths={"lane_log": str(root / "lanes/policy-validate/lane.log"), "lane_triage": str(root / "lanes/policy-validate/lane.triage.json")})):
                    (root / "lanes" / "policy-validate").mkdir(parents=True, exist_ok=True)
                    (root / "lanes" / "policy-validate" / "lane.log").write_text("ok\n", encoding="utf-8")
                    (root / "lanes" / "policy-validate" / "lane.triage.json").write_text("{}\n", encoding="utf-8")
                    with mock.patch.object(fg_required, "_check_working_tree_clean", side_effect=[None, SystemExit("working tree mutated")]):
                        argv = ["fg_required.py", "--global-budget-seconds", "480", "--strict", "--dry-run"]
                        with mock.patch("sys.argv", argv):
                            with self.assertRaises(SystemExit):
                                fg_required.main()
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(fg_required, "ARTIFACT_ROOT", root):
                ...
                failed = fg_required.LaneResult(
                    ...
                    False,
                    error="exit_2",
                    artifact_paths={"lane_log": str(lane_dir / "lane.log"), "lane_triage": str(lane_dir / "lane.triage.json")},
                )
                with mock.patch.object(fg_required, "_run_lane", return_value=failed):
                    checker = mock.Mock(side_effect=[None])
                    with mock.patch.object(fg_required, "_check_working_tree_clean", checker):
                        argv = ["fg_required.py", "--global-budget-seconds", "480", "--strict", "--dry-run"]
                        with mock.patch("sys.argv", argv):
                            rc = fg_required.main()
                self.assertEqual(rc, 1)
