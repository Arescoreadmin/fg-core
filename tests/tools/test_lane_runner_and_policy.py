...
def test_lane_runner_allowlist_rejects_unknown_lane(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("sys.argv", ["lane_runner.py", "--lane", "unknown"])
    with pytest.raises(SystemExit):
        lane_runner.main()


def test_lane_runner_shell_false_and_list_args_only(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_policy_validation_unknown_key_fails(tmp_path: Path) -> None:
    ...
    with pytest.raises(SystemExit):
        validate_policy._assert_unknown_keys("policy", {"version": 1, "unknown": True}, {"version"})
