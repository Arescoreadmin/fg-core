...
def test_policy_change_requires_justification_or_label(
...
def test_policy_change_allowed_with_justification_or_label(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.chdir(tmp_path)
    import subprocess
...
def test_policy_change_allowed_with_label_only(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.chdir(tmp_path)
    import subprocess
...
def test_policy_change_allowed_with_policy_context_body(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.chdir(tmp_path)
    import subprocess
    ...
    event.write_text(
        json.dumps(
            {
                "pull_request": {
                    "body": "Runtime baseline update: adjust quarantine guard and flake threshold for nightly lane stability.",
                    "labels": [],
                }
            }
        ),
