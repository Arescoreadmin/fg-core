...
SCRIPT = Path(__file__).resolve().parents[1] / "scripts" / "verify_bp_m3_001.py"
...
def _repo(tmp_path: Path) -> Path:
    ...
    _write(
        ...
        json.dumps({"BP-M3-001": "make bp-m3-001-gate"}) + "\n",
    )
...
def test_negative_proof(tmp_path: Path) -> None:
    ...
    report = json.loads(
        (repo / "artifacts/gates/bp_m3_001_report.json").read_text(encoding="utf-8")
    )
    assert any("missing ledger append-only trigger" in e for e in report["errors"])
