...
const STATUS_LABELS: Record<MsgraphRunStatus, string> = {
  ...
  scanning: 'Scanning SharePoint sites and OneDrive drives…',
...
export function SharepointScanPanel({ engagementId, onSuccess }: Props) {
  ...
  async function handleInitiate(e: React.FormEvent) {
    ...
    try {
      const initiated = await fieldAssessmentApi.initiateSharepointScan(engagementId, {
  ...
  return (
    <div className="space-y-4" aria-label="sharepoint-scan-panel">
      <p className="text-xs text-muted">
        Scans SharePoint sites and OneDrive drives for anonymous sharing links, external user access, and links without expiration. Up to 30 sites sampled.
      </p>
      {!runId && (
        <form onSubmit={handleInitiate} className="space-y-3">
          <div className="space-y-1">
            <Label htmlFor="sharepoint-tenant-id">Azure Tenant ID *</Label>
            <input
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="space-y-1">
              <Label htmlFor="sharepoint-operator-name">Operator Name</Label>
              <input
                id="sharepoint-operator-name"
                type="text"
                value={operatorName}
            ...
            <div className="space-y-1">
              <Label htmlFor="sharepoint-operator-org">Operator Org</Label>
              <input
                id="sharepoint-operator-org"
                type="text"
                value={operatorOrg}
          ...
          <p className="text-[11px] text-muted">
            Requires: Sites.Read.All, Files.Read.All, Directory.Read.All. Scans root-level items; deeply nested files are not inspected.
          </p>
          ...
          <Button type="submit" disabled={initiating || !azureTenantId.trim()} className="w-full sm:w-auto">
            {initiating ? 'Initiating…' : 'Run SharePoint & OneDrive Scan'}
          </Button>
        </form>
      )}
      {runId && runStatus && (
