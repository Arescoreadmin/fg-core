...
const STATUS_LABELS: Record<MsgraphRunStatus, string> = {
  ...
  scanning: 'Running OAuth inventory scan…',
...
export function OauthInventoryScanPanel({ engagementId, onSuccess }: Props) {
  ...
  async function handleInitiate(e: React.FormEvent) {
    ...
    try {
      const initiated = await fieldAssessmentApi.initiateOauthInventoryScan(engagementId, {
  ...
  return (
    <div className="space-y-4" aria-label="oauth-inventory-scan-panel">
      <p className="text-xs text-muted">
        Enumerates app registrations, service principals, and OAuth permission grants via MS Graph.
      </p>
      {!runId && (
        <form onSubmit={handleInitiate} className="space-y-3">
          <div className="space-y-1">
            <Label htmlFor="oauth-tenant-id">Azure Tenant ID *</Label>
            <input id="oauth-tenant-id" type="text" required value={azureTenantId}
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="space-y-1">
              <Label htmlFor="oauth-operator-name">Operator Name</Label>
              <input id="oauth-operator-name" type="text" value={operatorName}
            ...
            <div className="space-y-1">
              <Label htmlFor="oauth-operator-org">Operator Org</Label>
              <input id="oauth-operator-org" type="text" value={operatorOrg}
          ...
          <Button type="submit" disabled={initiating || !azureTenantId.trim()} className="w-full sm:w-auto">
            {initiating ? 'Initiating…' : 'Run OAuth Inventory Scan'}
          </Button>
        </form>
      )}
      {runId && runStatus && (
