...
const STATUS_LABELS: Record<MsgraphRunStatus, string> = {
  ...
  scanning: 'Scanning Entra ID Governance…',
...
export function EntraGovernanceScanPanel({ engagementId, onSuccess }: Props) {
  ...
  async function handleInitiate(e: React.FormEvent) {
    ...
    try {
      const initiated = await fieldAssessmentApi.initiateEntraGovernanceScan(engagementId, {
  ...
  return (
    <div className="space-y-4" aria-label="entra-governance-scan-panel">
      <p className="text-xs text-muted">
        Deep identity governance scan: PIM role assignments, Access Reviews, Identity Protection (P2), and Conditional Access policy posture.
      </p>
      {!runId && (
        <form onSubmit={handleInitiate} className="space-y-3">
          <div className="space-y-1">
            <Label htmlFor="entra-gov-tenant-id">Azure Tenant ID *</Label>
            <input
              id="entra-gov-tenant-id"
              type="text"
              required
              value={azureTenantId}
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="space-y-1">
              <Label htmlFor="entra-gov-operator-name">Operator Name</Label>
              <input
                id="entra-gov-operator-name"
                type="text"
                value={operatorName}
            ...
            <div className="space-y-1">
              <Label htmlFor="entra-gov-operator-org">Operator Org</Label>
              <input
                id="entra-gov-operator-org"
                type="text"
                value={operatorOrg}
          ...
          <p className="text-[11px] text-muted">
            Requires: Directory.Read.All, Policy.Read.All, AccessReview.Read.All, IdentityRiskyUser.Read.All, RoleEligibilitySchedule.Read.Directory.
            Identity Protection checks require Azure AD P2. Endpoints gracefully skip on 403 if license is absent.
          </p>
          ...
          <Button type="submit" disabled={initiating || !azureTenantId.trim()} className="w-full sm:w-auto">
            {initiating ? 'Initiating…' : 'Run Entra Governance Scan'}
          </Button>
        </form>
      )}
      {runId && runStatus && (
