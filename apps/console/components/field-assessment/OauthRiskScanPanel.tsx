...
const STATUS_LABELS: Record<MsgraphRunStatus, string> = {
  ...
  scanning: 'Analyzing OAuth grants and app permissions…',
...
export function OauthRiskScanPanel({ engagementId, onSuccess }: Props) {
  ...
  async function handleInitiate(e: React.FormEvent) {
    ...
    try {
      const initiated = await fieldAssessmentApi.initiateOauthRiskScan(engagementId, {
  ...
  return (
    <div className="space-y-4" aria-label="oauth-risk-scan-panel">
      <p className="text-xs text-muted">
        Deep OAuth risk analysis: illicit consent grants, over-privileged application permissions, AI tool data access, and unverified third-party publishers.
      </p>
      {!runId && (
        <form onSubmit={handleInitiate} className="space-y-3">
          <div className="space-y-1">
            <Label htmlFor="oauth-risk-tenant-id">Azure Tenant ID *</Label>
            <input
              id="oauth-risk-tenant-id"
              type="text"
              required
              value={azureTenantId}
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="space-y-1">
              <Label htmlFor="oauth-risk-operator-name">Operator Name</Label>
              <input
                id="oauth-risk-operator-name"
                type="text"
                value={operatorName}
            ...
            <div className="space-y-1">
              <Label htmlFor="oauth-risk-operator-org">Operator Org</Label>
              <input
                id="oauth-risk-operator-org"
                type="text"
                value={operatorOrg}
          ...
          <Button type="submit" disabled={initiating || !azureTenantId.trim()} className="w-full sm:w-auto">
            {initiating ? 'Initiating…' : 'Run OAuth Risk Scan'}
          </Button>
        </form>
      )}
      {runId && runStatus && (
