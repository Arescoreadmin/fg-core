...
import {
  fieldAssessmentApi,
  type MsgraphRunStatus,
  type MsgraphRunStatusResult,
...
const STATUS_LABELS: Record<MsgraphRunStatus, string> = {
  ...
  scanning: 'Running MS Graph scan…',
...
export function MsgraphScanPanel({ engagementId, onSuccess }: Props) {
  ...
  const isRunning = runId !== null && runStatus !== null && !TERMINAL_STATUSES.includes(runStatus.status);
  const isComplete = runStatus?.status === 'complete';
  const isFailed = runStatus?.status === 'failed';

  return (
    <div className="space-y-4" aria-label="msgraph-scan-panel">
      <div className="flex items-center justify-between">
        <p className="text-xs text-muted">
          Initiates a live MS Graph device-code scan and auto-imports the result.
        </p>
      </div>
      ...
      {!runId && (
        <form onSubmit={handleInitiate} className="space-y-3">
          <div className="space-y-1">
            <Label htmlFor="msgraph-tenant-id">Azure Tenant ID *</Label>
            <input
              id="msgraph-tenant-id"
              type="text"
              required
              value={azureTenantId}
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="space-y-1">
              <Label htmlFor="msgraph-operator-name">Operator Name</Label>
              <input
                id="msgraph-operator-name"
                type="text"
                value={operatorName}
            ...
            <div className="space-y-1">
              <Label htmlFor="msgraph-operator-org">Operator Org</Label>
              <input
                id="msgraph-operator-org"
                type="text"
                value={operatorOrg}
          ...
          <Button
            type="submit"
            disabled={initiating || !azureTenantId.trim()}
            className="w-full sm:w-auto"
          >
            {initiating ? 'Initiating…' : 'Run MS Graph Scan'}
          </Button>
        </form>
      )}
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-foreground">
              {STATUS_LABELS[runStatus.status]}
            </span>
          </div>
          ...
          {(userCode || runStatus.user_code) && !isComplete && !isFailed && (
            <div className="rounded border border-border bg-surface-2 p-4 space-y-3">
              ...
              <p className="text-xs text-muted">
                1. Open the link below in a browser, then enter the code when prompted.
              </p>
              ...
              {(verificationUri ?? runStatus.verification_uri) && (
                ...
                >
                  Open {verificationUri ?? runStatus.verification_uri}
          ...
          {isComplete && runStatus.scan_result_id && (
            <Alert variant="success">
              <AlertDescription>
                Scan imported — result ID:{' '}
                <code className="font-mono text-xs">{runStatus.scan_result_id}</code>
              </AlertDescription>
            </Alert>
          )}
          ...
          {isFailed && (
            <Alert variant="destructive">
              <AlertDescription>
                {runStatus.error ?? 'Scan failed — check server logs for details.'}
              </AlertDescription>
            </Alert>
          )}
