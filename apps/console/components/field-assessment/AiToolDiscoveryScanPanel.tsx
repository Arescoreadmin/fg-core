...
const STATUS_LABELS: Record<MsgraphRunStatus, string> = {
  ...
  scanning: 'Discovering AI-connected applications…',
...
export function AiToolDiscoveryScanPanel({ engagementId, onSuccess }: Props) {
  ...
  async function handleInitiate(e: React.FormEvent) {
    ...
    try {
      const initiated = await fieldAssessmentApi.initiateAiToolDiscoveryScan(engagementId, {
  ...
  return (
    <div className="space-y-4" aria-label="ai-tool-discovery-scan-panel">
      <p className="text-xs text-muted">
        Discovers AI-connected applications from Microsoft Graph evidence: app registrations, service principals, OAuth grants, app roles, sign-in timestamps, and publisher verification. No prompt, document, email, or browsing content is collected.
      </p>
      {!runId && (
        <form onSubmit={handleInitiate} className="space-y-3">
          <div className="space-y-1">
            <Label htmlFor="ai-tool-discovery-tenant-id">Azure Tenant ID *</Label>
            <input
              id="ai-tool-discovery-tenant-id"
              type="text"
              required
              value={azureTenantId}
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="space-y-1">
              <Label htmlFor="ai-tool-discovery-operator-name">Operator Name</Label>
              <input
                id="ai-tool-discovery-operator-name"
                type="text"
                value={operatorName}
            ...
            <div className="space-y-1">
              <Label htmlFor="ai-tool-discovery-operator-org">Operator Org</Label>
              <input
                id="ai-tool-discovery-operator-org"
                type="text"
                value={operatorOrg}
                onChange={(e) => setOperatorOrg(e.target.value)}
                placeholder="FrostGate"
                className="flex h-9 w-full rounded border border-border bg-surface-2 px-3 py-2 text-sm text-foreground placeholder:text-muted focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary"
                disabled={initiating}
              />
            </div>
          ...
          <p className="text-[11px] text-muted">
            Requires: Application.Read.All, Directory.Read.All, AuditLog.Read.All. Detects illicit consent grant attack pattern, ChatGPT/Claude/Copilot data access, and tenant-wide write permissions.
          </p>
          ...
          <Button type="submit" disabled={initiating || !azureTenantId.trim()} className="w-full sm:w-auto">
            {initiating ? 'Initiating…' : 'Run AI Tool Discovery Scan'}
          </Button>
        </form>
      )}
      {runId && runStatus && (
          ...
          {(userCode || runStatus.user_code) && !isComplete && !isFailed && (
            <div className="rounded border border-border bg-surface-2 p-4 space-y-3">
              ...
              {(verificationUri || runStatus.verification_uri) && (
                <a
                  href={verificationUri ?? runStatus.verification_uri ?? '#'}
                  target="_blank"
