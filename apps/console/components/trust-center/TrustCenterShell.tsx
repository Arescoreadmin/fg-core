...
const MCIM_ID = 'MCIM-18.6-TRUST-SHELL';
const AUTHORITY = 'Trust Center Authority';
const sourceOfTruth = '/api/core/control-tower/snapshot';
const drillDown = '/trust-center';

export interface TrustCenterShellProps {
  mcimId: string;
  authority: string;
  capability: string;
...
export default function TrustCenterShell({
  mcimId,
  authority,
  capability,
  sourceOfTruth: sot,
  ...
  return (
    <Card className={className} aria-label={`trust-center-panel-${mcimId}`}>
      ...
      <CardContent>
        ...
        {metaOpen && (
          <div
            ...
            aria-label="trust-center-panel-metadata"
          >
            ...
            {domain && (
              <div className="flex justify-between">
                <span className="font-semibold uppercase tracking-wide">Domain</span>
                <span>{domain}</span>
              </div>
            )}
            <div className="flex justify-between">
