...
PROTECTED_PATH_PREFIXES = ("auth/", "db/", "migrations/", "security/", "planes/", "contracts/")
...
def validate_critical_coverage(invariants: list[dict[str, object]]) -> None:
    for inv in invariants:
        if inv.get("severity") == "critical" and not inv.get("enforced_by"):
            raise SystemExit(f"critical invariant missing enforcing tests: {inv.get('id')}")


def validate_path_mapping(changed_paths: list[str], mapping: dict[str, list[str]]) -> None:
    for path in changed_paths:
        if path.startswith(PROTECTED_PATH_PREFIXES) and not any(path.startswith(k) or path == k for k in mapping):
            raise SystemExit(f"protected path lacks invariant mapping: {path}")




def validate_route_prefix_registrations(changed_paths: list[str], mapping: dict[str, list[str]]) -> None:
    for path in changed_paths:
...
def main() -> int:
    ...
    parser.add_argument("--path-map", default="tools/testing/policy/path_to_invariants.yaml")
