...
_RULES: tuple[TriageRule, ...] = (
    TriageRule(
        ...
        ("make fg-contract", "python tools/testing/contracts/check_contract_drift.py"),
        ("contracts/", "tools/testing/contracts/check_contract_drift.py"),
        ("tools/testing/README.md",),
    ),
    TriageRule(
        TriageCategory.DUPLICATE_ROUTES,
        re.compile(r"duplicate route|route conflict", re.IGNORECASE),
        0.95,
        "Duplicate route registration detected.",
        ("python tools/ci/check_route_inventory.py",),
        ("api/main.py", "tools/ci/check_route_inventory.py"),
        ("CONTRACT.md",),
    ),
    TriageRule(
        TriageCategory.PLANE_REGISTRY_DRIFT,
        re.compile(r"plane registry|ownership_map", re.IGNORECASE),
        0.9,
        "Plane registry and ownership map diverged.",
    ...
    TriageRule(
        ...
        re.compile(r"scope|forbidden|permission", re.IGNORECASE),
        0.88,
    ...
    TriageRule(
        ...
        re.compile(r"tenant|cross-tenant|isolation", re.IGNORECASE),
        0.9,
    ...
    TriageRule(
        ...
        re.compile(r"\brls\b|row level security", re.IGNORECASE),
        0.9,
    ...
    TriageRule(
        ...
        re.compile(r"ssrf|metadata endpoint|169\.254\.169\.254", re.IGNORECASE),
        0.94,
    ...
    TriageRule(
        ...
        re.compile(r"migration|ddl|sql|rollback", re.IGNORECASE),
        0.89,
    ...
    TriageRule(
        ...
        re.compile(r"time budget exceeded|timeout|budget-exhausted", re.IGNORECASE),
        0.97,
        "Lane exceeded strict runtime budget.",
    ...
    TriageRule(
        TriageCategory.FLAKE_SUSPECTED,
        re.compile(r"flaky|intermittent|rerun|oscillat", re.IGNORECASE),
        0.86,
        "Flaky test suspected from inconsistent outcomes.",
        ("python tools/testing/harness/flake_detect.py --lane fg-flake-detect",),
        (
            "tools/testing/policy/flaky_tests.yaml",
            "artifacts/testing/flake-report.json",
        ),
        ("tools/testing/README.md",),
    ),
)
...
def _top_frames(lines: list[str]) -> list[str]:
    frames = [
        line.strip()
        for line in lines
        if line.lstrip().startswith("File ") or "::" in line
    ]
...
def _classify(lines: list[str], lane: str = "unknown") -> dict[str, object]:
    ...
    report["evidence"]["stable_hash"] = hashlib.sha256(
        json.dumps(report, sort_keys=True).encode("utf-8")
    ).hexdigest()
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate deterministic triage output from test logs"
    )
    ...
    parser.add_argument("--lane", default="unknown", help="Lane name")
