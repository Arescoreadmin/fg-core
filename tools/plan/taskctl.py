...
def build_indexes(plan: dict[str, Any]) -> tuple[dict[str, Any], dict[str, str], list[str]]:
...
def get_current(plan: dict[str, Any], state: dict[str, Any]) -> tuple[dict[str, Any], str, str]:
...
def validate_cmd(args: argparse.Namespace) -> int:
    ...
    if failures:
        print(f"\nValidation failed. See {summary_path.relative_to(ROOT)}", file=sys.stderr)
        return 1
...
def complete_cmd(args: argparse.Namespace) -> int:
    ...
    if args.require_validation:
        ...
        if not summary.get("success"):
            raise SystemExit(f"Cannot complete {current_task_id}: latest validation did not pass.")
...
def parser() -> argparse.ArgumentParser:
    ...
    c.add_argument("--require-validation", action="store_true", help="Require latest validation artifact to pass")
